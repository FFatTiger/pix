...
## 107. Cross-platform CP-38 — named-pipe DACL before listen

- Branch/base: `feat/cross-platform-g0-baseline` / `66127cd`.
...
- Worker connection now drains an already-reaped child after attaching `exit` (VS Code child-wrapper pattern). Darwin + POSIX `detached:true` can miss the `exit` event; a missed exit emptied the event loop and `--test-force-exit` cancelled the rest of `worker-process`. stdout is stopped only after its pipe drains (bounded grace), so a final frame written right before exit is not lost.
- The test helpers `onceMessage`/`onceExit` had a TDZ bug: the `const unsub` was referenced inside the listener while `subscribe()`/`onExit()` were still replaying buffered events synchronously. The throw was swallowed by the connection's replay try/catch, `clearTimeout` already ran, and the promise never settled — force-exiting the file. Switched to `let unsub = () => {}`.
- WSL pays ~40s per fresh child to load the Pi SDK module graph, so `concurrent starts` gets a 120s ping deadline (still seconds on macOS/Windows). The `live pid lock` fixture now carries our own `start` identity — Linux classifies a live pid without one as obstructed.
- macOS sessiond now green end-to-end: `realpath(tmpdir())` breaks Darwin `sun_path` (104), so tests keep the short operational path and canonicalize only the walk; hang fixtures hold a referenced interval (an empty stdin `end` listener still lets the loop drain); the trampoline forwards stdin manually instead of `pipe()`; `onceMessage`/`onceExit` use `let unsub = () => {}` (sync replay TDZ); worker stdout drains before stop. Windows 269/0/66, macOS 333/0/2, WSL socket-publish 20/0/0.
- Hang fixtures must observe stdin `end` on Darwin. `resume()` alone still exits when the parent closes the pipe, so close-escalation tests never reached SIGKILL. The trampoline also cannot `pipe()` stdin: Darwin auto-end of the grandchild stdin tears the trampoline down before SIGKILL.

---

## 117. Cross-platform CP-55–CP-58 — ledger identity without destructive ownership reuse

- Branch/base: `feat/cross-platform-g0-baseline` / `a740603`.
- Ledger v2 file-ID schema is explicitly won't-do. Trusted and managed v1 `*Dev/*Ino` fields are complete-pair optional audit metadata; Windows production writers omit Node pseudo-POSIX identity. Half-pairs, zero/unsafe values, wrong kind/version, sparse/corrupt data still fail closed; schema remains version 1 and no dependency or migration rewrite is added.
- Trusted-root rehydrate uses canonical real directories, durable AllowedRoot containment, and non-main/non-prunable Git topology. Successful corroboration captures fresh runtime `FileIdentity`; unavailable Git evidence fails startup before ledger rewrite or memory authorization.
- Managed-worktree rehydrate separates **workspace access** from **destructive ownership**. Path + non-prunable repository list + checkout-side common/admin/repo match + pre/post runtime identity can restore access/history, but `managedByPix`/DELETE remains true only for a current-Host `recordCreated()` runtime identity token. Host restart and remove/re-add at the same path do not inherit the former `worktreeId` delete authority. A future explicit reclaim flow is a separate product decision.
- Git/tool/policy unavailable preserves exact ledger evidence and publishes no authority; only a completed authoritative negative drops the exact record. This retains §39/§41's invariant that Git topology alone never grants DELETE.
- Independent reviewer identified prunable, unavailable, writer, pair-atomicity, and documentation issues; fixes were applied. An architecture oracle independently ruled that same-path re-add cannot inherit destructive ownership because `managed-worktrees.json` is deletion authority rather than a workspace bookmark.
