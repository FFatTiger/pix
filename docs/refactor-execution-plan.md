...
| ID | 工作包 | 状态 | 依赖 | 交付 |
|---|---|---|---|---|
| `D1` | Sessions Read Path | `IN_PROGRESS` | `M2`, Session Catalog Adapter | D1A DONE；D1B-1 `723fac4` Thinking、D1B-2 `83a3859` Bash History/Live、D1B-3 `665e31a` selected history visible-branch normalized JSON export DONE；后续为更完整只读会话能力 |
...
6. M2：DONE；M3 当前 checkpoint：D1A只读历史、D1B Thinking/Bash展示与selected history visible-branch normalized JSON export、D2-P0/P1轻命令、D3A Files/Git/Worktrees只读工作区、D3B端到端只读Catalog均已合并；下一纵向切片进入D2 capability扩张或D3A Mutation重要节点
```
