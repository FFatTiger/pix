...
8. `Host files route Node v24/@types-node 类型兼容`：IN_REVIEW（待 Grok），实现 `785c88a`（base 2cfa98a；Grok 独立验证 verdict PENDING，不提前 DONE）。此前阻塞 root build/Startup E2E 的 Host `files.ts` 类型错误（ReadStream `data` 回调显式 `Buffer` 过窄：TS2345；`value instanceof File` 谓词因 buffer/undici File 名义冲突非法：TS2677/TS2339）已修复：ReadStream 明确 `encoding: null` 并以 `string | Buffer` 兼容回调（string 用 TextEncoder UTF-8 编码、Buffer 用 `new Uint8Array(chunk)` 拷贝入队，不暴露池化 ArrayBuffer 的 byteOffset/byteLength 之外）；Upload 改为最小结构接口 `UploadFile` + 基于 `FormData#getAll` 实际 union 的 fail-closed guard（排除 string，校验 name / safe 非负 size / arrayBuffer），不用名义 global File/instanceof/any，未关 strict。仅改 `packages/host/src/routes/files.ts` + `packages/host/test/resources.test.mjs`。验证：Host build/typecheck/test（263 total，0 fail）/check:boundaries、root build/typecheck/check:architecture、`git diff 2cfa98a..HEAD --check` 均 PASS；Startup E2E 在干净无 gate 环境（文档化 env override `PIX_AUTH_DISABLED=true`）PASS 2/2；raw 本机被仓库外 `~/.pi/pix.json`（2026-08-12 写入，auth.password+disabled:false）的 gate 401 阻塞，属基线/环境阻塞（gate/auth/security 源码与 base 逐字节一致，E2E 失败路径未执行 files.ts，直连探针已取证；未绕过认证）。
```
