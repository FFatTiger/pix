...
| `CP-47` | sessiond POSIX fixture 对齐 VS Code：短 `tmpdir()` + canonicalize walk + lock 0600；worker 对已 reap 的 child drain `exit`（Darwin detached 不丢事件） | `IN_PROGRESS` | `packages/sessiond` | `CP-46` | macOS sessiond 全绿；不放松 generic symlink fail-closed |
