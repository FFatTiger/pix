...
async function main() {
  ...
  try {
    ...
    // Models are a GLOBAL catalog now: no cwd required; legacy/unknown query
    // parameters fail closed instead of being silently ignored.
    const globalModels = await fetch(`${origin}/v1/models`);
    assert.equal(globalModels.status, 200);
    const legacyModels = await fetch(`${origin}/v1/models?cwd=${encodeURIComponent(project)}`);
