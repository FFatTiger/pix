 ...
 *   7. GET /v1/sessions?page=&pageSize= + /v1/projects — independent true pages /
 ...
 *  11. Phase 6 planned: history outside AllowedRoots is readable with
 *      `workspaceAccess=history_only`; models/send stay blocked. A deleted
 *      registered root classifies `unavailable`.
 *
 ...
 */
async function writeSessionJsonl(agentDir, cwd, sessionId, messages, options = {}) {
  ...
  const header = JSON.stringify({
    ...
    version: 3,
    id: sessionId,
    timestamp: FIXTURE_ENTRY_TS,
    cwd: resolve(cwd),
    ...(options.parentSession === undefined ? {} : { parentSession: options.parentSession }),
  });
  await writeFile(file, [
    header,
    ...messages.map((m) => fixtureMessage(m.id, m.parentId, m.role, m.text, m.timestamp)),
    ...(options.extraEntries ?? []).map((entry) => JSON.stringify(entry)),
  ].join("\n") + "\n");
  return file;
}
...
async function main() {
  ...
  try {
    ...
    let fixtureParentFile;
    for (const [id, offset] of [[FIXTURE_P1, 5000], [FIXTURE_P2, 4000], [FIXTURE_P3, 3000], [FIXTURE_P4, 2000]]) {
      const file = await writeSessionJsonl(agentDir, projectCwd, id, linearPair(id, BASE + offset));
      if (id === FIXTURE_P1) fixtureParentFile = file;
    }
    assert.ok(fixtureParentFile);
    const FIXTURE_SUBAGENT = "e2e-subagent-hidden";
    await writeSessionJsonl(
      agentDir,
      projectCwd,
      FIXTURE_SUBAGENT,
      linearPair(FIXTURE_SUBAGENT, BASE + 6000),
      { parentSession: fixtureParentFile },
    );
    const FIXTURE_CUSTOM_CHILD = "e2e-custom-child-hidden";
    await writeSessionJsonl(
      agentDir,
      projectCwd,
      FIXTURE_CUSTOM_CHILD,
      linearPair(FIXTURE_CUSTOM_CHILD, BASE + 7000),
      {
        extraEntries: [{
          type: "custom",
          id: "e2e-custom-provenance",
          parentId: null,
          timestamp: FIXTURE_ENTRY_TS,
          customType: "pix-fork-provenance",
          data: { parentSessionId: FIXTURE_P1, forkPointEntryId: `${FIXTURE_P1}-m1` },
        }],
      },
    );
    ...
    // 2b. True independent numbered Sessions / Projects resources.
    const idsOf = (res) => res.body.sessions.map((s) => s.sessionId);
    const page1 = await get("/v1/sessions?page=1&pageSize=2");
    assert.equal(page1.status, 200);
    assert.deepEqual(idsOf(page1), [FIXTURE_P1, FIXTURE_P2]);
    assert.equal(idsOf(page1).includes(FIXTURE_SUBAGENT), false, "parent-linked subagent must be hidden before page slicing");
    assert.equal(idsOf(page1).includes(FIXTURE_CUSTOM_CHILD), false, "custom-provenance child must be hidden before page slicing");
    assert.deepEqual({ page: page1.body.page, pageSize: page1.body.pageSize, total: page1.body.total, totalPages: page1.body.totalPages }, { page: 1, pageSize: 2, total: 6, totalPages: 3 });
    const page2 = await get("/v1/sessions?page=2&pageSize=2");
    assert.equal(page2.status, 200);
    assert.deepEqual(idsOf(page2), [FIXTURE_P3, FIXTURE_P4], "page 2 replaces page 1; it never repeats a prefix");
    const page3 = await get("/v1/sessions?page=3&pageSize=2");
    assert.deepEqual(idsOf(page3), [FIXTURE_BRANCH, sessionId]);
    const outOfRange = await get("/v1/sessions?page=4&pageSize=2");
    assert.deepEqual(idsOf(outOfRange), []);
    assert.equal(outOfRange.body.page, 4);
    const projectPage = await get("/v1/projects?page=1&pageSize=10");
    assert.equal(projectPage.status, 200);
    assert.equal(projectPage.body.page, 1);
    assert.equal(projectPage.body.pageSize, 10);
    assert.ok(Array.isArray(projectPage.body.projects));
    const projectSessions = await get(`/v1/sessions?page=1&pageSize=2&projectRoot=${encodeURIComponent(projectCwd)}`);
    assert.equal(projectSessions.status, 200);
    assert.equal(projectSessions.body.total, 6);
    assert.equal(projectSessions.body.sessions.some((session) => session.sessionId === FIXTURE_SUBAGENT), false);
    assert.equal(projectSessions.body.sessions.some((session) => session.sessionId === FIXTURE_CUSTOM_CHILD), false);
    const hiddenChildDetail = await get(`/v1/sessions/${FIXTURE_SUBAGENT}`);
    assert.equal(hiddenChildDetail.status, 200, "temporary browse hiding must not delete child JSONL or break exact reads");
    assert.equal(hiddenChildDetail.body.session.parentSessionId, FIXTURE_P1);
    const hiddenCustomDetail = await get(`/v1/sessions/${FIXTURE_CUSTOM_CHILD}`);
    assert.equal(hiddenCustomDetail.status, 200);
    assert.equal(hiddenCustomDetail.body.session.parentSessionId, FIXTURE_P1);
    assert.equal(hiddenCustomDetail.body.session.forkPointEntryId, `${FIXTURE_P1}-m1`);
    for (const q of [
      "page=0&pageSize=2", "page=01&pageSize=2", "page=1e3&pageSize=2",
      "page=1&pageSize=0", "page=1&pageSize=101", "limit=2", "offset=1",
    ]) {
    ...
    assert.equal(ctxDefault.body.context.contextTokens, 2, "HTTP must preserve the SDK's latest branch usage estimate");
    const rawContext = await rpc.call("sessions.context", { sessionId: FIXTURE_BRANCH, deferThinking: true, deferMedia: true });
    const deferredContext = await get(`/v1/sessions/${FIXTURE_BRANCH}/context?deferThinking=1&deferMedia=1&limit=1`);
    assert.equal(deferredContext.status, 200);
    assert.equal(deferredContext.body.context.contextTokens, rawContext.contextTokens, "Host must not drop the validated sessiond numerator");
    assert.deepEqual(deferredContext.body.context.settings, rawContext.settings);
    assert.equal(deferredContext.body.context.leafId, rawContext.leafId);
    assert.equal(deferredContext.body.context.entries.length, 1);
    ...
    assert.equal("entries" in p4ReadAfter.body.session, false, "detail stays header-only; history belongs to context pages");
    const p4ContextAfter = await get(`/v1/sessions/${FIXTURE_P4}/context`);
    assert.equal(p4ContextAfter.status, 200);
    assert.deepEqual(p4ContextAfter.body.context.entries.map((e) => e.entryId), [`${FIXTURE_P4}-m1`, `${FIXTURE_P4}-m2`], "rename must preserve the session history");
    ...
    // 5d. Phase 6 planned: history outside AllowedRoots is readable as
    //     history_only (models/send stay Host-enforced). A missing path under
    //     the live root classifies unavailable. Catalog visibility is never a
    //     live grant and never expands roots.
    {
      const { mkdtemp: makeTemp, rm: remove } = await import("node:fs/promises");
      const { join: joinPath } = await import("node:path");
      const outsideCwd = await makeTemp(join(tmpdir(), "pix-e2e-outside-"));
      const outsideId = "e2e-outside-0001";
      await writeSessionJsonl(agentDir, outsideCwd, outsideId, linearPair(outsideId, BASE + 50));
      // The suite has already warmed the adapter's bounded list cache. Exact
      // detail lookup intentionally performs the documented one-shot refresh
      // on a warm index miss; prove detail first, then prove the refreshed row
      // is catalog-visible. No sleep and no production TTL override.
      const outsideDetail = await get(`/v1/sessions/${outsideId}`);
      assert.equal(outsideDetail.status, 200);
      assert.deepEqual(outsideDetail.body.session.workspaceAccess, {
        state: "history_only",
        reason: "outside_allowed_roots",
      });
      const mixedList = await get("/v1/sessions");
      assert.equal(mixedList.status, 200);
      const outsideRow = mixedList.body.sessions.find((s) => s.sessionId === outsideId);
      assert.ok(outsideRow, "history outside AllowedRoots must remain catalog-visible");
      assert.deepEqual(outsideRow.workspaceAccess, {
        state: "history_only",
        reason: "outside_allowed_roots",
      });
      const outsideContext = await get(`/v1/sessions/${outsideId}/context`);
      assert.equal(outsideContext.status, 200, "history-only JSONL remains readable");
      // Models are a GLOBAL catalog: readable regardless of workspace roots.
      const modelsGlobal = await get(`/v1/models`);
      assert.equal(modelsGlobal.status, 200, "global models catalog must stay readable");
      await remove(outsideCwd, { recursive: true, force: true });

      const missingId = "e2e-missing-root-0001";
      const missingCwd = joinPath(projectCwd, "deleted-workspace");
      await writeSessionJsonl(agentDir, missingCwd, missingId, linearPair(missingId, BASE + 40));
      const missingDetail = await get(`/v1/sessions/${missingId}`);
      assert.equal(missingDetail.status, 200, "deleted-path history remains catalog-readable");
      assert.equal(missingDetail.body.session.workspaceAccess.state, "unavailable");
      assert.ok(
        missingDetail.body.session.workspaceAccess.reason === "missing"
          || missingDetail.body.session.workspaceAccess.reason === "deleted",
      );
    }
