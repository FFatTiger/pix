...
import type { AgentMessage, ContextUsage, StreamingAgentMessage } from "./messages.js";
...
/**
 * Coherent context payload for {@link RuntimeStateChangedEvent} (context-usage
 * consistency). Captured from ONE backend read so `model`, the committed
 * branch `leafId`, and `contextUsage` are atomic — a consumer applying the
 * payload can never observe a model from one point in time next to a usage
 * from another. `contextUsage: null` is the honest UNKNOWN (missing window or
 * post-compaction) and MUST clear any previously projected value.
 */
export interface RuntimeStateChangedContext {
  /** Current execution model; null when the backend reports none. */
  model: ModelRef | null;
  /** Committed branch leaf; null when the session has no entries yet. */
  leafId: string | null;
  /** Current context usage; null = unknown (clears stale projections). */
  contextUsage: ContextUsage | null;
}

/**
 * State changed; the authoritative state is available via getSnapshot(). The
 * optional `context` payload (context-usage consistency) lets signal-only
 * consumers keep their cheaper semantics while payload-aware consumers update
 * {model, leafId, contextUsage} atomically without a snapshot round-trip.
 */
export interface RuntimeStateChangedEvent extends RuntimeEventBase {
  type: "runtime_state_changed";
  context?: RuntimeStateChangedContext;
}

/**
 * The session's branch pointer / on-disk identity changed out-of-band of the
 * ordinary turn stream (Phase 5A): a successful session-tree navigate, or a
 * compaction terminal that rewrote the branch. Carries the AUTHORITATIVE
 * post-change session cwd and the active session-tree leaf id when the
 * backend exposes one (`undefined` when the session has no entries yet —
 * never a guessed leaf). Consumers use the leaf as a fence: the same leaf is
 * already continuous; a different leaf requires a history/live-tail rebase.
 *
 * Emitted ONLY after the change is authoritatively applied (a rejected
 * navigate/compact never emits it). Pure product semantics: no epochs, event
 * ids or resume cursors (Protocol-layer concepts stamped downstream).
 */
export interface SessionChangedEvent extends RuntimeEventBase {
  type: "session_changed";
  /** Authoritative session working directory after the change. */
  cwd: string;
