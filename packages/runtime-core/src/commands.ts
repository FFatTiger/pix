...
export interface GenerateSessionTitleCommand {
  ...
  /**
   * Additive Protocol v2 field: the model the title generation must use.
   * Absent = the session's current model (older-producer compatibility).
   * Unknown/unavailable models fail closed inside the adapter.
   */
...
/**
 * Commands that only read the current runtime projection. These remain
 * correlated requests for transport purposes, but must not consume the
 * sessiond epoch's finite mutation at-most-once admission ledger.
 */
export const READ_ONLY_RUNTIME_COMMAND_TYPES = [
  "get_state",
  "get_session_stats",
  "get_last_assistant_text",
  "get_tools",
  "get_commands",
] as const satisfies readonly RuntimeCommandType[];

export type ReadOnlyRuntimeCommandType = (typeof READ_ONLY_RUNTIME_COMMAND_TYPES)[number];
export type RuntimeReadCommand = Extract<RuntimeCommand, { type: ReadOnlyRuntimeCommandType }>;
