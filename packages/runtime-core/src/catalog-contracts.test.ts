...
// Workspace access is an EXACT tri-state. Adding a fourth state or dropping one fails this.
const _workspaceAccessStateExact: IsExact<
  WorkspaceAccessState,
  "authorized" | "history_only" | "unavailable"
> = true;
const _workspaceAccessReasonExact: IsExact<
  WorkspaceAccessReason,
  | "allowed_root"
  ...
  | "malformed"
> = true;
const _workspaceAccessShapeExact: IsExact<
  WorkspaceAccess,
  { state: WorkspaceAccessState; reason: WorkspaceAccessReason }
> = true;
...
test("workspace access state is exactly the tri-state {authorized,history_only,unavailable}", () => {
  assert.deepEqual([...WORKSPACE_ACCESS_STATES], [
    "authorized",
    "history_only",
    "unavailable",
  ]);
  ...
  assert.deepEqual([...WORKSPACE_ACCESS_REASONS_BY_STATE.authorized], ["allowed_root"]);
  assert.deepEqual([...WORKSPACE_ACCESS_REASONS_BY_STATE.history_only], [
    "outside_allowed_roots",
    "symlink_escape",
  ]);
  assert.deepEqual([...WORKSPACE_ACCESS_REASONS_BY_STATE.unavailable], [
    "missing",
    "deleted",
    "unreadable",
    "unresolvable",
...
test("combineWorkspaceAccess is fail-closed: unavailable > history_only > authorized", () => {
  ...
  const malformed: WorkspaceAccess = { state: "unavailable", reason: "malformed" };
  assert.deepEqual(combineWorkspaceAccess(authorized, history), history);
  assert.deepEqual(combineWorkspaceAccess(history, authorized), history);
  assert.deepEqual(combineWorkspaceAccess(history, missing), missing);
  assert.deepEqual(combineWorkspaceAccess(authorized, missing), missing);
  assert.deepEqual(combineWorkspaceAccess(history, escape), escape);
  assert.deepEqual(combineWorkspaceAccess(missing, malformed), malformed);
  assert.doesNotThrow(() => JSON.parse(JSON.stringify(authorized)));
});
...
test("a read-only session catalog object exposes no renameSession", () => {
  const catalog: SessionCatalogPort = {
    ...
    readSessionThinking: () => Promise.reject(new Error("not implemented")),
  ...
  assert.ok(keys.includes("readSessionThinking"), "the exact-identity deferred-thinking read stays on the catalog");
});

// The catalog read surface is exact: true pages + prepare, persisted reads and
// legacy full-list/delete compatibility. No rename/runtime lifecycle rides along.
const _catalogExact: IsExact<
  keyof SessionCatalogPort,
  | "prepare"
  | "listSessionPage"
  | "listSessions"
  | "readSession"
