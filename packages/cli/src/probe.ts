...
import { PROTOCOL_VERSION, SESSIOND_BUILD_IDENTITY, evaluateSessiondBuild, type SessiondBuildIncompatibilityReason } from "@fffattiger/pix-protocol";
 ...
 * Phase 7A build fence).
 *
 * - `current`: a POSITIVELY authenticated `system.hello` returned the current
 *   protocol version AND a build identity that passes the explicit Phase 7A
 *   compatibility matrix (product, protocol, sessiond/Worker/adapter contract
 *   generations, capability fingerprint — see
 *   `packages/protocol/src/build.ts`) ⇒ the daemon may be reused as-is.
 * - `staleBuild`: a POSITIVELY authenticated current-protocol daemon whose
 *   build identity is missing (pre-fence build) or fails the matrix. The SAME
 *   protocol major does NOT mean implementation compatible, so the daemon is
 *   never silently reused; it MAY be replaced — but only through the
 *   authenticated, instance-fenced shutdown path, never via a PID signal.
 ...
 * - `unverifiable`: the daemon is NOT positively authenticated as current,
 *   staleBuild, or knownLegacy — transient hello timeout, auth failure,
 *   malformed/unparseable response (including a hello carrying a MALFORMED
 *   build block, which fails the strict hello schema), unreachable, or a
 *   positively authenticated but UNSUPPORTED version (future major / 0 /
 *   other numeric). The daemon MUST be preserved; callers return a retryable
 *   or fixed operator error instead of touching authority.
 */
export type SessiondCompatibility =
  | { state: "current" }
  | { state: "staleBuild"; reason: SessiondBuildIncompatibilityReason }
 ...
 * `system.hello`. Protocol v2 stale-daemon safety + the Phase 7A build fence:
 * this is the ONE place the CLI decides whether a running daemon is current,
 * stale-build, known-legacy, or unverifiable, so `ensureSessiond` can reuse,
 * replace, or preserve authority without ever guessing. Never throws.
 */
export async function probeSessiondCompatibility(
  ...
  //    For a current-protocol daemon the Phase 7A build matrix then decides
  //    whether that daemon's BUILD is reusable — same protocol major alone is never
  //    sufficient. A missing build block (pre-fence dist) or a value mismatch
  //    classifies as staleBuild (positively authenticated, replaceable); a
  //    MALFORMED block fails the strict hello schema below and surfaces as
  //    unverifiable (preserved — never destroyed on a guess).
  ...
  try {
    ...
    if (result.protocolVersion !== PROTOCOL_VERSION) {
      ...
      const build = evaluateSessiondBuild(result.build, SESSIOND_BUILD_IDENTITY);
      if (build.state === "compatible") return { state: "current" };
      return { state: "staleBuild", reason: build.reason };
    }
  } catch (error) {
    currentError = error;
  }
  ...
  if (legacy.ok) {
    ...
    // A current-version answer over the LEGACY envelope can never be accepted
    // as "current": this client cannot evaluate that daemon's Phase 7A build
    // identity over the legacy envelope, and a current-protocol daemon has no
    // business answering it. Classify unverifiable (preserved), never reused.
