...
/** Format an IPv6 bind address as a valid URL authority. */
export function formatHostForUrl(hostname: string): string {
...
export async function runHost(
  ...
  // exact-session authorizer. AllowedRoots stays the only workspace policy.
  const sessiondClient = new SessiondRpcClient({ endpoint: location.paths.endpoint, secret });
  ...
  const url = `http://${formatHostForUrl(options.hostname)}:${handle.port}`;
