...
import { resolveAllowedHosts, resolveCatalogAgentDir, formatHostForUrl } from "../src/commands/host-runner.js";
...
test("trusted hosts merge the bind address with operator configuration", () => {
  assert.deepEqual(resolveAllowedHosts("0.0.0.0", {
    PIX_HOSTNAME: " pix.example.com ",
    PIX_ALLOWED_HOSTS: "pix.example.com, pix.lan, ,192.168.0.10",
  }), ["0.0.0.0", "pix.example.com", "pix.lan", "192.168.0.10"]);
  assert.deepEqual(resolveAllowedHosts("127.0.0.1", {}), ["127.0.0.1"]);
});

test("formatHostForUrl brackets IPv6 authorities", () => {
  assert.equal(formatHostForUrl("::"), "[::]");
  assert.equal(formatHostForUrl("2001:db8::1"), "[2001:db8::1]");
  assert.equal(formatHostForUrl("192.168.0.10"), "192.168.0.10");
...
test("runHost fails closed (exit 1) when the sessiond secret is missing", async () => {
  ...
  const fixture = buildClientFixture();
  const prevClientDist = process.env.PIX_CLIENT_DIST;
  const paths = sessiondPaths(dir);
  const location: SessiondLocation = { directory: dir, endpoint: paths.endpoint, paths };
  try {
    process.env.PIX_CLIENT_DIST = fixture;
    const code = await runHost(location, { hostname: "127.0.0.1", port: 0, open: false });
    assert.equal(code, 1);
  } finally {
    if (prevClientDist === undefined) delete process.env.PIX_CLIENT_DIST;
    else process.env.PIX_CLIENT_DIST = prevClientDist;
    rmSync(fixture, { recursive: true, force: true });
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("production host-runner wires the Host authorizer from the trusted sessiond client and AllowedRoots", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const source = readFileSync(join(here, "..", "..", "src", "commands", "host-runner.ts"), "utf8");
  assert.match(source, /createRuntimeWorkspaceAuthorizer/);
  assert.match(source, /runtimeWorkspaceAuthorizer/);
  assert.match(source, /new SessiondRpcClient/);
  assert.match(source, /roots: production\.deps\.allowedRoots/);
  assert.equal(source.includes("assert("), false);
