...
export interface SessiondRpcHandler {
  ...
  watchRunning?(): PreparedRunningWatch;
  submitTurn?(params: SessiondMethodParams["runtime.submitTurn"]): Promise<PreparedTurnSubmission>;
}
...
export class SessiondRpcServer {
    ...
    socket.on("data", (chunk) => {
      ...
      while (true) {
        ...
        void this.process(
          ...
          writer,
          line,
          (next) => { attachment?.close(); attachment = next; },
          (next) => { runningWatch?.close(); runningWatch = next; },
          (next) => { turnSubmission?.close(); turnSubmission = next; },
        ).catch((error) => this.log(`[sessiond] rpc request process failed: ${describeSafe(error)}`));
      }
    });
    socket.on("close", () => { attachment?.close(); runningWatch?.close(); turnSubmission?.close(); writer.close(); this.sockets.delete(socket); });
    socket.on("error", () => { attachment?.close(); runningWatch?.close(); turnSubmission?.close(); writer.close(); this.sockets.delete(socket); });
  }
  ...
  private async process(
    ...
    line: string,
    setAttach: (attachment: PreparedAttachment) => void,
    setRunningWatch: (watch: PreparedRunningWatch) => void,
    setTurnSubmission: (submission: PreparedTurnSubmission) => void,
  ): Promise<void> {
    let input: unknown;
    ...
    if (request.method === "runtime.submitTurn" && this.options.handler.submitTurn) {
      let submission: PreparedTurnSubmission;
    ...
    if (request.method === "runtime.watchRunning" && this.options.handler.watchRunning) {
      let watch: PreparedRunningWatch;
      try {
        watch = this.options.handler.watchRunning();
      } catch (error) {
        await this.writeFailureSafely(writer, request.id, request.method, toBoundaryProtocolError(error), error);
        return;
      }
...
export class SessiondRpcClient {
  ...
  async submitTurn(params: SessiondMethodParams["runtime.submitTurn"], onPush: (push: import("@fffattiger/pix-protocol").SessiondTurnStatusPush) => void | Promise<void>): Promise<SessiondRpcSubscription<SessiondMethodResult["runtime.submitTurn"]>> {
    ...
    return new Promise((resolve, reject) => {
      ...
      let submitted = false;
      let deliveryReady = false;
      const pendingPushes: import("@fffattiger/pix-protocol").SessiondTurnStatusPush[] = [];
      let settled = false;
      let closedResolve: (() => void) | undefined;
      const closed = new Promise<void>((settle) => { closedResolve = settle; });
      const settleClosed = () => { pendingPushes.length = 0; const settle = closedResolve; closedResolve = undefined; settle?.(); };
      const timer = setTimeout(() => { socket.destroy(); if (!settled) { settled = true; reject(new SessiondError("timeout", "sessiond submit turn timed out", true)); } }, this.options.timeoutMs ?? 15_000);
      const fail = (error: Error) => { clearTimeout(timer); socket.destroy(); if (!settled) { settled = true; reject(error); } };
      socket.on("connect", () => socket.write(`AUTH ${this.options.secret}\n`));
      socket.on("data", (chunk) => {
        buffered += chunk.toString("utf8");
        while (true) {
          ...
          if (!submitted) {
            const response = SessiondRpcResponseSchema.safeParse(value);
            if (!response.success || response.data.id !== id || response.data.method !== "runtime.submitTurn") return fail(new SessiondError("invalid_request", "mismatched submit turn response"));
            if (!response.data.ok) return fail(new SessiondError(response.data.error.code, response.data.error.message, response.data.error.retryable, response.data.error.details));
      ...
      socket.on("error", (error) => { if (!settled) fail(error); else settleClosed(); });
      socket.on("close", () => { if (!settled) fail(new SessiondError("unavailable", "sessiond submit turn closed", true)); else settleClosed(); });
    });
  }

  async watchRunning(onPush: (push: import("@fffattiger/pix-protocol").SessiondRunningStatePush) => void | Promise<void>): Promise<SessiondRpcSubscription<SessiondMethodResult["runtime.watchRunning"]>> {
    const id = crypto.randomUUID();
    ...
    return new Promise((resolve, reject) => {
      ...
      let watching = false;
      let deliveryReady = false;
      const pendingPushes: import("@fffattiger/pix-protocol").SessiondRunningStatePush[] = [];
      let settled = false;
      let closedResolve: (() => void) | undefined;
      const closed = new Promise<void>((settle) => { closedResolve = settle; });
      const settleClosed = () => {
        pendingPushes.length = 0;
        const settle = closedResolve;
        closedResolve = undefined;
        settle?.();
      };
      const timer = setTimeout(() => { socket.destroy(); if (!settled) { settled = true; reject(new SessiondError("timeout", "sessiond running watch timed out", true)); } }, this.options.timeoutMs ?? 10_000);
      const fail = (error: Error) => { clearTimeout(timer); socket.destroy(); if (!settled) { settled = true; reject(error); } };
      socket.on("connect", () => socket.write(`AUTH ${this.options.secret}\n`));
      socket.on("data", (chunk) => {
        buffered += chunk.toString("utf8");
        while (true) {
          ...
          if (!watching) {
            const response = SessiondRpcResponseSchema.safeParse(value);
            if (!response.success || response.data.id !== id || response.data.method !== "runtime.watchRunning") return fail(new SessiondError("invalid_request", "mismatched running watch response"));
            if (!response.data.ok) return fail(new SessiondError(response.data.error.code, response.data.error.message, response.data.error.retryable, response.data.error.details));
            watching = true; settled = true; clearTimeout(timer);
            resolve({ response: response.data.result, closed, close: () => { settleClosed(); socket.destroy(); } });
            queueMicrotask(() => {
              deliveryReady = true;
              for (const push of pendingPushes.splice(0)) Promise.resolve(onPush(push)).catch(() => socket.destroy());
      ...
      socket.on("error", (error) => { if (!settled) fail(error); else settleClosed(); });
      socket.on("close", () => { if (!settled) fail(new SessiondError("unavailable", "sessiond running watch closed", true)); else settleClosed(); });
    });
  }
