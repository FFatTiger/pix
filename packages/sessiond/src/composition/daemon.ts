...
import type { ProjectCatalogPort, SessionCatalogPort, SessionLocatorPort, SessionMutationPort } from "@fffattiger/pix-runtime-core";
...
export interface DaemonOptions {
  ...
  /** Override independent project pages; null disables the read surface. */
  projectCatalog?: ProjectCatalogPort | null;
  /**
...
function buildDependencies(directory: string, options: DaemonOptions): SessiondDependencies {
  ...
  const projectCatalog = options.projectCatalog === undefined
    ? (options.sessionCatalog === undefined ? defaultPorts.projectCatalog : null)
    : options.projectCatalog;
  if (projectCatalog) deps.projectCatalog = projectCatalog;
...
export async function startDaemon(options: DaemonOptions = {}): Promise<DaemonHandle> {
  ...
  try {
    ...
    const dependencies = buildDependencies(directory, options);
    // Build/reconcile the disposable materialized browse index BEFORE the RPC
    // endpoint is published. After this boundary numbered page requests perform
    // SQLite-only reads (zero readdir/lstat/JSONL/Git work).
    if (options.workerFactory === undefined) {
      await dependencies.sessionCatalog?.prepare?.();
      await dependencies.projectCatalog?.prepare?.();
    }
    const service = new SessiondService(dependencies, options.serviceOptions);
