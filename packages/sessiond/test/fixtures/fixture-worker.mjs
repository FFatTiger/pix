...
// Connection-layer tests only require a SCHEMA-VALID build block (Phase 7A
// fence): any positive contract generations + a 64-hex fingerprint parse.
// Identity-level fence tests run in-process against the real
// WORKER_BUILD_IDENTITY and never use this fixture.
const FIXTURE_BUILD = {
  workerContract: 3,
  adapterContract: 1,
...
function ready(id = "init:fixture", sessionId = "sess-fixture", sessiondBuild) {
  // worker.init carries the larger SessiondBuild shape; worker.ready must carry
  // only the strict WorkerBuild subset. Echoing the whole init block would be
  // rejected for unknown fields and would hide framing behavior behind a
  // schema error.
  const build = sessiondBuild && typeof sessiondBuild === "object"
    ? {
        workerContract: sessiondBuild.workerContract,
        adapterContract: sessiondBuild.adapterContract,
        fingerprint: sessiondBuild.fingerprint,
      }
    : FIXTURE_BUILD;
  writeFrame({
    type: "worker.ready",
    id,
    payload: { sessionId, workerStatus: "ready", features: ["runtime.read-rpc.v1", "runtime.submit-turn.v1"], build },
  });
}
...
switch (mode) {
  ...
  case "stderr-flood": {
    const secret =
      ...
      "path=/Users/alice/.pi/secrets/key.json " +
  ...
  case "split-frames": {
    const frame = JSON.stringify({
      type: "worker.ready",
      id: "split",
      payload: { sessionId: "sess-split", workerStatus: "ready", features: ["runtime.read-rpc.v1", "runtime.submit-turn.v1"], build: FIXTURE_BUILD },
    }) + "\n";
    let offset = 0;
    const step = () => {
  ...
  case "coalesced": {
    const a = JSON.stringify({
      type: "worker.ready",
      id: "c1",
      payload: { sessionId: "sess-c1", workerStatus: "ready", features: ["runtime.read-rpc.v1", "runtime.submit-turn.v1"], build: FIXTURE_BUILD },
    });
  ...
  case "crlf": {
    const frame = JSON.stringify({
      type: "worker.ready",
      id: "crlf",
      payload: { sessionId: "sess-crlf", workerStatus: "ready", features: ["runtime.read-rpc.v1", "runtime.submit-turn.v1"], build: FIXTURE_BUILD },
    });
    ...
    drainStdin(
      (msg) => {
        if (msg && msg.type === "worker.init") {
          ready(msg.id, msg.payload?.sessionId ?? "sess-fixture", msg.payload?.build);
        ...
        } else if (msg && msg.type === "worker.rotateEpoch") {
          // Phase 5B: acknowledge a whole-epoch rotation with the exact
          // correlated ok result (fixture workers are always idle).
          const { sessionId, fromEpoch, toEpoch } = msg.payload ?? {};
          writeFrame({
            type: "worker.rotateEpochResult",
            id: msg.id,
            payload: { sessionId, fromEpoch, toEpoch, ok: true },
          });
        }
      },
      () => process.exit(0),
