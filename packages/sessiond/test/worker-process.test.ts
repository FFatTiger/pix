...
function onceMessage(
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const registered = connection.subscribe((message) => {
    ...
    // subscribe() may synchronously replay an early buffered frame before it
    // returns its unsubscribe function. Remove that listener after registration
    // instead of referencing a temporal-dead-zone `const unsub` in the replay.
...
function onceExit(
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const registered = connection.onExit((exit) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      unsubscribe();
    ...
    unsubscribe = registered;
    // onExit() also synchronously replays an early buffered exit.
    if (settled) registered();
  });
}
...
test("redactStderr strips secrets and paths", () => {
  const raw =
    "Authorization: Bearer supersecrettokenvalue path=/Users/alice/.pi/key.json OPENAI_API_KEY=sk-abcdefghijklmnop";
...
test("StderrRing redacts API key / Bearer / path / token across every 2- and 3-chunk split", () => {
  const secrets = [
    ...
    {
      label: "path",
      full: "loading /Users/alice/.pi/secrets/key-material-abcdef.json\n",
      secretBody: "/Users/alice/.pi/secrets/key-material-abcdef.json",
    },
    {
      label: "token-assign",
...
test("StderrRing 40-round flood stays bounded and redacted", () => {
  ...
  assert.equal(snap.includes("/Users/alice/.pi/secrets"), false);
...
test("early message is buffered until subscribe", async () => {
  ...
  try {
    // Deterministically observe the exact internal buffer condition that this
    // white-box process test owns. A wall-clock sleep can fire before stdout is
    // scheduled under load and, on assertion failure, leave the child alive.
    await waitUntil(() => (
      connection as unknown as { earlyMessages: WorkerToSessiondMessage[] }
    ).earlyMessages.some((message) => message.type === "worker.ready"));
...
test("early exit is buffered until onExit", async () => {
  ...
  try {
    await waitUntil(() => (
      connection as unknown as { earlyExit?: WorkerExit }
    ).earlyExit !== undefined);
