...
import { PROTOCOL_VERSION, RUNTIME_READ_RPC_FEATURE, SESSIOND_RPC_METHODS } from "@fffattiger/pix-protocol";
...
test("daemon serves system.ping and system.hello", async () => {
  ...
  try {
    ...
    assert.equal(hello.capabilities?.includes(RUNTIME_READ_RPC_FEATURE), true);
...
test("sessions.read projects a bounded header before RPC framing even when catalog detail is huge", async () => {
  ...
  const h = await activationHarness({
    readSession(sessionId) {
      return {
        ...
        title: "Large session",
        entries: [{ entryId: "entry-1", message: { role: "user", content: oversizedText } }],
      };
    },
  });
  try {
    const result = await h.rpc.call("sessions.read", { sessionId: "large" });
    assert.equal(result.sessionId, "large");
    assert.equal(result.title, "Large session");
    assert.equal("entries" in result, false, "complete transcript must stay on bounded sessions.context");
    assert.equal(h.readCalls(), 1);
    assert.equal((await h.rpc.call("system.ping", {})).pong, true, "oversized catalog detail must not break the RPC socket");
  } finally {
...
// ---------------------------------------------------------------------------
// Direct source-history parity over the daemon's REAL default adapter:
// deferral flags must shrink the context BEFORE RPC framing (a >5MiB base64
// tool-result image may never reach the bounded RPC writer), the exact-identity
// sessions.thinking RPC must resolve a deferred block, and both identity error
// classes must stay fail-closed at the RPC boundary. Zero workers throughout.
// ---------------------------------------------------------------------------

test("daemon real adapter: deferMedia omits a >5MiB tool-result image before RPC framing; thinking RPC resolves deferred blocks", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "sessiond-defer-agent-"));
  const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
  process.env.PI_CODING_AGENT_DIR = agentDir;
  const dir = await tempDir();
  try {
    const cwd = join(agentDir, "workspace");
    await mkdir(cwd, { recursive: true });
    // >5MiB decoded payload as base64 (~6.99MiB of wire text): far beyond the
    // 2MiB RPC frame bound, so it may only ever travel via the deferred seam.
    const hugeBase64 = Buffer.alloc(5 * 1024 * 1024 + 1024, 1).toString("base64");
    const seeded = seedSessionForTests({
      cwd,
      userText: "screenshot the panel",
      assistantText: "on it",
      assistantThinking: "planning the capture",
      toolResultImage: { data: hugeBase64, mimeType: "image/png" },
    });
    const sessionId = seeded.sessionId;
    const assistantEntry = seeded.assistantEntryId;

    const workers = new FakeWorkerFactory({ readyDelayMs: 0 });
    const handle = await startDaemon({ directory: dir, workerFactory: workers, serviceOptions: { idleTimeoutMs: 0 } });
    const rpc = client(handle);
    try {
      // Deferred context: thinking placeholder + truthful media-omission summary.
      const context = await rpc.call("sessions.context", { sessionId, deferThinking: true, deferMedia: true });
      const serialized = JSON.stringify(context);
      assert.ok(serialized.length < 1024 * 1024, "deferred context must stay far below the RPC frame bound");
      assert.equal(serialized.includes(hugeBase64.slice(0, 4096)), false, "the >5MiB base64 payload must never reach the RPC response");
      assert.ok(
        serialized.includes("1 tool result image omitted from initial history payload: image/png, ~"),
        "the truthful source omission summary replaces the media",
      );
      const assistant = context.entries.find((entry) => entry.entryId === assistantEntry);
      const firstBlock = assistant?.message.role === "assistant" ? assistant.message.content[0] : undefined;
      assert.deepEqual(firstBlock, { type: "thinking", thinking: "", deferred: true });

      // Subsequent ping on the SAME connection stays healthy (no oversized frame).
      assert.equal((await rpc.call("system.ping", {})).pong, true);

      // Exact-identity thinking resolution over the same zero-worker catalog.
      const thinking = await rpc.call("sessions.thinking", { sessionId, entryId: assistantEntry, blockIndex: 0 });
      assert.deepEqual(thinking, { sessionId, entryId: assistantEntry, blockIndex: 0, thinking: "planning the capture" });

      // Fail-closed identity errors at the RPC boundary: canonical not_found for
      // a non-thinking block and an unknown entry; invalid_input for a bad index.
      const isNotFoundRpcError = (error: unknown): boolean =>
        error instanceof SessiondError && error.code === "not_found";
      await assert.rejects(() => rpc.call("sessions.thinking", { sessionId, entryId: assistantEntry, blockIndex: 1 }), isNotFoundRpcError);
      await assert.rejects(() => rpc.call("sessions.thinking", { sessionId, entryId: "entry-does-not-exist", blockIndex: 0 }), isNotFoundRpcError);
      // Malformed params are rejected by the strict client schema before any
      // request is written; no adapter/catalog lookup can occur.
      await assert.rejects(
        () => rpc.call("sessions.thinking", { sessionId, entryId: assistantEntry, blockIndex: -1 }),
      );

      // Zero workers: pure JSONL history reads.
      assert.equal(workers.starts, 0);
      assert.deepEqual((await rpc.call("runtime.listRunning", {})).sessions, []);
      // The daemon connection is still healthy after every probe.
      assert.equal((await rpc.call("system.ping", {})).pong, true);
    } finally {
      await handle.shutdown();
    }
...
test("daemon real default adapter: JSONL offline rename, zero workers, same path/id/history/context, immediate read/list", async () => {
  ...
  try {
    ...
    try {
      ...
      const list = await rpc.call("sessions.list", { page: 1, pageSize: 50 });
