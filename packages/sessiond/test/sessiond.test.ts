...
test("createRequestId is idempotent while distinct creates remain distinct and may start concurrently", async () => {
  ...
  assert.equal(first.lastEventId, 1, "create returns the exact post-startup journal cursor");
  assert.equal(duplicate.lastEventId, first.lastEventId);
  assert.equal(second.lastEventId, 1);
  assert.equal(workers.starts, 2);
  await service.shutdown();
});

test("create-path rekey synchronizes the Worker authority epoch exactly once before ready exposure", async () => {
  const { service, workers } = harness({ worker: { discoveredSessionId: "real-rekey" } });
  const created = await service.create({ createRequestId: "rekey-epoch", cwd: "/a", projectRoot: "/a" });
  assert.equal(created.sessionId, "real-rekey");
  const worker = workers.workers[0]!;
  const init = worker.sent.find((message) => message.type === "worker.init");
  const rotations = worker.sent.filter((message) => message.type === "worker.rotateEpoch");
  assert.ok(init?.type === "worker.init");
  assert.equal(rotations.length, 1);
  assert.equal(rotations[0]?.type === "worker.rotateEpoch" && rotations[0].payload.fromEpoch, init.payload.epoch);
  assert.equal(rotations[0]?.type === "worker.rotateEpoch" && rotations[0].payload.toEpoch, created.epoch);
  await service.shutdown();
});

test("create-path rekey epoch sync rejection fails startup closed", async () => {
  const { service } = harness({ worker: { discoveredSessionId: "real-rekey-fail", rotateEpochBehavior: "reject" } });
  await assert.rejects(service.create({ createRequestId: "rekey-epoch-fail", cwd: "/a", projectRoot: "/a" }));
...
test("turn busy flips broadcast authoritative global busy session ids", async () => {
  ...
  workers.workers[0]!.emitEvent({ type: "agent_end", sessionId: "s" });
  await wait();
  assert.equal(service.listRunning().sessions.find((item) => item.sessionId === "s")?.workerStatus, "busy", "agent_end is an attempt boundary, not a prompt terminal");
  workers.workers[0]!.emitEvent({ type: "agent_settled", sessionId: "s" });
  await wait();
  ...
  assert.ok(ended, "agent_settled broadcasts an empty busy set");

  attached.close();
  await service.shutdown();
});

test("global running watch converges busy to idle without any session attachment", async () => {
  const { service, workers } = harness();
  await service.activate("s");
  const watch = service.prepareRunningWatch();
  assert.deepEqual(watch.result.sessionIds, ["s"]);
  assert.deepEqual(watch.result.busySessionIds, []);
  assert.equal(service.diagnostics().subscribers, 0, "global watch is not a per-session subscriber");
  ...
  assert.ok(states[0]!.revision > watch.result.revision);
  assert.deepEqual(states[0]!.busySessionIds, ["s"]);
  assert.ok(states[1]!.revision > states[0]!.revision);
  assert.deepEqual(states[1]!.busySessionIds, []);
  watch.close();
  await service.shutdown();
});

test("running watch baseline precedes buffered updates and starts zero workers", async () => {
  const empty = harness();
  const emptyWatch = empty.service.prepareRunningWatch();
  assert.deepEqual(emptyWatch.result, { revision: 0, sessionIds: [], busySessionIds: [] });
  assert.equal(empty.workers.starts, 0);
  assert.equal(empty.service.diagnostics().subscribers, 0);
  emptyWatch.close();
  ...
  workers.workers[0]!.emitEvent({ type: "agent_start", sessionId: "s" });
  await wait();
  const pushes: Array<{ revision: number; busySessionIds: string[] }> = [];
  await watch.flushTo((push) => { pushes.push(push.state); });
  assert.equal(pushes.length, 1);
  assert.ok(pushes[0]!.revision > watch.result.revision);
  assert.deepEqual(pushes[0]!.busySessionIds, ["s"]);
  watch.close();
  workers.workers[0]!.emitEvent({ type: "agent_end", sessionId: "s" });
  workers.workers[0]!.emitEvent({ type: "agent_settled", sessionId: "s" });
  await wait();
  const reconnected = service.prepareRunningWatch();
  assert.deepEqual(reconnected.result.busySessionIds, []);
  assert.ok(reconnected.result.revision > pushes[0]!.revision);
  reconnected.close();
  await service.shutdown();
});
...
test("navigate same-id second caller waits for deferred authority refresh with one worker.command", async () => {
  const { service, workers } = harness({
    worker: { snapshot: navigateSnapshot("s"), holdPostCommandSnapshots: true },
    service: { commandTimeoutMs: 2_000 },
  });
  await service.activate("s");
  const worker = workers.workers[0]!;
  ...
  const first = service.command("s", command);
  // Deterministic gate: wait until the post-command authority refresh is IN
  // FLIGHT and HELD (the singleflight entry is installed before that refresh is
  // dispatched, so the first caller is provably blocked on it). No wall-clock
  // guess about how long the refresh takes.
  await worker.waitForHeldSnapshot();
  let firstSettled = false;
  void first.then(() => { firstSettled = true; });
  assert.equal(firstSettled, false, "original caller must not settle while the authority refresh is held");

  const second = service.command("s", command);
  let secondSettled = false;
  void second.then(() => { secondSettled = true; });
  // Drain microtasks so the second caller deterministically reaches the
  // singleflight join (runExclusive + the synchronous join check are both
  // microtask-bound). The held gate guarantees neither caller can settle yet.
  ...
  // Only releasing the gate may settle the joined callers — the first result
  // cannot settle until the authoritative refresh is released.
...
test("ordinary rename does not issue extra post-command snapshot refresh", async () => {
  ...
  const snapshotsAfter = workers.workers[0]!.sent.filter((item) => item.type === "worker.getSnapshot").length;
  assert.equal(snapshotsAfter, snapshotsBefore, "rename must not need an extra Worker snapshot refresh");
  const projected = service.getSnapshot("s");
  assert.equal(projected.state.sessionName, "Renamed", "confirmed rename must update the sessiond projection used by attach/local reads");
  await service.shutdown();
});
...
test("read-only snapshot and catalog operations never activate a worker", async () => {
  ...
  await app.handle("sessions.list", { page: 1, pageSize: 50 });
...
test("sessions.list forwards page/pageSize/cwd/projectRoot and sessions.context forwards leafId to the catalog", async () => {
  ...
  const catalog: SessionCatalogPort = {
    async listSessionPage(request) {
      listCalls.push({ ...request });
      return { sessions: [], page: request.page, pageSize: request.pageSize, total: 0, totalPages: 0, catalogRevision: 1 };
    },
    async listSessions() { return []; },
    async readSession(sessionId) { return { sessionId, cwd: "/workspace", projectRoot: "/workspace", entries: [] }; },
...
test("projects.list forwards an independent numbered page and starts zero workers", async () => {
  const calls: Array<{ page: number; pageSize: number }> = [];
  const workers = new FakeWorkerFactory();
  const service = new SessiondService({
    ...
    activationContext: { async resolve(_id, _loc, cwd) { return { cwd: cwd ?? "/w", projectRoot: cwd ?? "/w" }; } },
    workerFactory: workers,
    projectCatalog: {
      async listProjectPage(request) {
        calls.push(request);
        return { projects: [{ projectRoot: "/p", representativeCwd: "/p", sessionCount: 3, latestActivity: 1 }], page: request.page, pageSize: request.pageSize, total: 1, totalPages: 1, catalogRevision: 2 };
      },
    },
  }, { idleTimeoutMs: 0 });
  try {
    const result = await new SessiondApplication(service).handle("projects.list", { page: 2, pageSize: 10 });
    assert.deepEqual(calls, [{ page: 2, pageSize: 10 }]);
    assert.equal(result.projects[0]?.projectRoot, "/p");
    assert.equal(workers.starts, 0);
  } finally { await service.shutdown(); }
});

test("sessions.context forwards defer flags and sessions.thinking resolves exact identity fail-closed", async () => {
  const contextCalls: Array<Record<string, unknown>> = [];
  const thinkingCalls: Array<{ sessionId: string; entryId: string; blockIndex: number }> = [];
  const catalog: SessionCatalogPort = {
    async listSessions() { return []; },
    async readSession(sessionId) { return { sessionId, cwd: "/workspace", projectRoot: "/workspace", entries: [] }; },
    async readSessionContext(sessionId, options) {
      contextCalls.push({
        ...
        ...(options?.deferThinking === undefined ? {} : { deferThinking: options.deferThinking }),
        ...(options?.deferMedia === undefined ? {} : { deferMedia: options.deferMedia }),
      });
      return { sessionId, entries: [], pageInfo: { hasMore: false } };
    },
    async readSessionThinking(sessionId, entryId, blockIndex) {
      thinkingCalls.push({ sessionId, entryId, blockIndex });
      if (entryId === "entry-missing") throw makeRuntimeError("not_found", "entry not found");
      return { sessionId, entryId, blockIndex, thinking: "resolved thought" };
    },
    async readSessionTree(sessionId) { return { sessionId, roots: [], entryCount: 0 }; },
    async deleteSession() {},
  };
  ...
  const app = new SessiondApplication(service);
  try {
    // Defer flags pass through 1:1; omitted flags stay omitted (full inline payload).
    await app.handle("sessions.context", { sessionId: "s-1", deferThinking: true, deferMedia: true });
    await app.handle("sessions.context", { sessionId: "s-2" });
    assert.deepEqual(contextCalls, [
      { sessionId: "s-1", deferThinking: true, deferMedia: true },
      { sessionId: "s-2" },
    ]);

    // Exact-identity thinking resolution echoes the identity verbatim.
    const resolved = await app.handle("sessions.thinking", { sessionId: "s-1", entryId: "entry-9", blockIndex: 3 });
    assert.deepEqual(resolved, { sessionId: "s-1", entryId: "entry-9", blockIndex: 3, thinking: "resolved thought" });
    assert.deepEqual(thinkingCalls, [{ sessionId: "s-1", entryId: "entry-9", blockIndex: 3 }]);

    // Fail-closed: an unknown entry surfaces as the canonical not_found error.
    await assert.rejects(
      () => app.handle("sessions.thinking", { sessionId: "s-1", entryId: "entry-missing", blockIndex: 0 }),
      (error: unknown) => isRuntimeError(error) && error.code === "not_found",
    );
  } finally {
    await service.shutdown();
  }
...
test("RPC authenticates locally and rejects the wrong secret", async (t) => {
  ...
  try {
    ...
    void worker;
    subscription.close();
    // Closing the attach stream is observed server-side asynchronously: wait on
    // the exact lifecycle completion (client `closed` promise) AND the
    // server-observed subscriber removal before asserting diagnostics — never a
...
test("RPC running watch delivers response baseline before pushes and is independent of attach", async (t) => {
  if (process.platform === "win32") return t.skip("unix socket test");
  const directory = await mkdtemp(join(tmpdir(), "sessiond-running-watch-"));
  const endpoint = join(directory, "rpc.sock");
  const { service, workers } = harness();
  const server = new SessiondRpcServer({ endpoint, secret: "a".repeat(40), handler: new SessiondApplication(service) });
  await server.listen();
  try {
    ...
    await client.call("runtime.activate", { sessionId: "watch-s" });
    const states: Array<{ revision: number; busySessionIds: string[] }> = [];
    const watch = await client.watchRunning((push) => { states.push(push.state); });
    assert.deepEqual(watch.response.sessionIds, ["watch-s"]);
    assert.deepEqual(watch.response.busySessionIds, []);
    const attach = await client.attach({ sessionId: "watch-s" }, async () => {});
    attach.close();
    workers.workers[0]!.emitEvent({ type: "agent_start", sessionId: "watch-s" });
    workers.workers[0]!.emitEvent({ type: "agent_end", sessionId: "watch-s" });
    workers.workers[0]!.emitEvent({ type: "agent_settled", sessionId: "watch-s" });
    await wait(20);
    assert.deepEqual(states.map((state) => state.busySessionIds), [["watch-s"], []]);
    assert.ok(states[0]!.revision > watch.response.revision);
    watch.close();
    await watch.closed;
  } finally {
    await server.close().catch(() => {});
