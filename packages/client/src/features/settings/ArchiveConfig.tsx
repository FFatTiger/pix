...
export function ArchiveConfig() {
  ...
  const sessions = useQuery({
    ...createQueryOptions(http).sessions.list(),
    enabled: canBrowseSessions,
  });
  const sessionById = new Map((sessions.data?.sessions ?? []).map((session) => [session.sessionId, session]));
