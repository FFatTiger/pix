...
/**
 * Project trust dialog (source: upstream desktop app components/ProjectTrustDialog.tsx).
 *
 * pix honesty rule: the confirm ("Trust project") action is rendered only when
 * a real trust-write surface is passed in (`onConfirmAction`). The Host today
 * exposes a read-only trust projector (GET /v1/trust) with no trust mutation,
 * so the dialog renders informationally (Cancel-only) until that capability
 * exists — no write button is fabricated.
 */
export function ProjectTrustDialog({
  ...
  error: string | null;
  onCancelAction: () => void;
  onConfirmAction?: (() => void) | undefined;
}) {
  const { t } = useI18n();
