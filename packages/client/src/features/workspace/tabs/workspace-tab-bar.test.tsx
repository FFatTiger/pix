...
describe("WorkspaceTabBar — accessible tabs", () => {
  ...
  it("right-click opens the tab menu with bulk-close actions wired to handlers", () => {
    const closeOther = vi.fn();
    const closeRight = vi.fn();
    const closeAll = vi.fn();
    const tabs = [sessionTab("A"), sessionTab("B"), fileTab("c")];
    mount({
      tabs,
      activeTabId: tabs[1]!.id,
      onCloseOtherTabs: closeOther,
      onCloseTabsToRight: closeRight,
      onCloseAllTabs: closeAll,
    });

    const second = screen.getAllByRole("tab")[1]!;
    fireEvent.contextMenu(second, { clientX: 40, clientY: 12 });
    ...
    expect(closeOther).toHaveBeenCalledWith(tabs[1]!.id);

    fireEvent.contextMenu(second, { clientX: 40, clientY: 12 });
    fireEvent.click(screen.getByRole("menuitem", { name: "Close tabs to the right" }));
    expect(closeRight).toHaveBeenCalledWith(tabs[1]!.id);
  ...
  it("disables bulk-close items that would be no-ops", () => {
    const tabs = [sessionTab("A"), sessionTab("B")];
    mount({ tabs, activeTabId: tabs[0]!.id, onCloseOtherTabs: vi.fn(), onCloseTabsToRight: vi.fn(), onCloseAllTabs: vi.fn() });
    // Last tab: "close right" is a no-op.
    fireEvent.contextMenu(screen.getAllByRole("tab")[1]!, { clientX: 40, clientY: 12 });
