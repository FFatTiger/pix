 ...
 *    {@link ExactRuntimeApi.sendExtensionUiInput} — the exact controller's
 *    dedicated FIFO lane preserves exact per-key order while the final-response
 *    slot stays independent; Close sends `\x03` exactly like the source.
 ...
 *    {@link ExactRuntimeApi.respondExtensionUi} final-response slot.
 *
...
export function ExtensionRequests({ live, composerTextareaRef }: ExtensionRequestsProps) {
  ...
  // component renders nothing — it never admits/attaches during render and never
  // invents component transport state. The existing `live` prop + capability
  // gating and every action/error/DOM behavior are unchanged.
  const exact = useSelectedRuntime();
  ...
  useLayoutEffect(() => {
    if (sessionIdRef.current !== (exact?.sessionId ?? null)) {
      sessionIdRef.current = exact?.sessionId ?? null;
  ...
  const interactive = activeInteractiveRequests(gated ? (exact?.snapshot ?? null) : null);
  ...
  }, [first, gated, exact?.sessionId, exact?.attached]);
