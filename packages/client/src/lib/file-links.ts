...
export function resolveLocalFileHref(
  ...
  // pix adapter: /v1/ is this app's internal route prefix (the source
  // excluded its own app-internal prefixes). Such hrefs are app endpoints,
  // and are never treated as file paths.
  if (lowerHref.startsWith("/v1/")) return null;
