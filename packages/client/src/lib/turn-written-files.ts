...
export function extractTurnWrittenFiles(
  ...
  for (const block of content) {
    ...
    const rawPath = readToolPath(block.input as Record<string, unknown> | undefined);
    if (!rawPath) continue;
    ...
    const stats = isEditToolName(block.toolName)
      ...
      : { additions: countWriteLines(block.input as Record<string, unknown> | undefined), deletions: 0 };
