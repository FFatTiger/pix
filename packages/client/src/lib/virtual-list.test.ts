...
describe("computeVirtualWindow — deterministic window content for a given scrollTop", () => {
  ...
  it("uses a synthetic viewport around a non-zero anchor before viewport measurement", () => {
    const w = computeVirtualWindow({
      count: 100,
      sizeAt: FIXED(40),
      scrollTop: 40 * 80,
    ...
    const indexes = w.items.map((item) => item.index);
    expect(indexes[0]).toBe(78);
    expect(indexes).toContain(80);
    expect(indexes.at(-1)).toBe(96);
  });
