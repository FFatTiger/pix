...
describe("API domain response parsing", () => {
  ...
  it("rejects malformed sessions", async () => {
    await expect(createSessionsApi(client({ sessions: [{ sessionId: "s" }] })).list({ page: 1, pageSize: 50 })).rejects.toMatchObject({ code: "INVALID_RESPONSE" });
  });

  it("parses the Host DELETE / PATCH success envelope", async () => {
    ...
    // Title generation has NO HTTP surface (typed runtime command instead),
    // so no autoName method exists to drift against a nonexistent Host route.
    expect((createSessionsApi(client({ ok: true })) as unknown as Record<string, unknown>).autoName).toBeUndefined();
  });
