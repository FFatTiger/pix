...
import { createGateApi, type GateLoginInput, type GatePasswordChangeInput } from "./gate";
import { createPreferencesApi, type PreferencesPatchInput } from "./preferences";
...
import { primeSessionPageTitleData } from "./session-list";
...
function primeSessionTitle(queryClient: QueryClient, id: string, title: string): void {
  ...
  for (const query of queryClient.getQueryCache().getAll()) {
    ...
    if (isQueryKeyPrefix(key, listPrefix)) {
      queryClient.setQueryData(key, (current: unknown) => primeSessionPageTitleData(current, id, title));
      continue;
    }
...
export function createMutationOptions(http: HttpClient, queryClient: QueryClient) {
  ...
  const preferencesApi = createPreferencesApi(http);
  ...
  return {
    sessions: {
      ...
      remove: () => ({
        mutationKey: ["pix", "sessions", "remove"] as const,
        mutationFn: (id: string) => sessions.remove(id),
        onSuccess: (_data: unknown, id: string) => invalidate(
          queryClient,
          queryKeys.sessions.lists,
          queryKeys.projects.all,
          queryKeys.sessions.byId(id),
        ),
      }),
    },
    gate: {
      ...
      changePassword: () => ({ mutationKey: ["pix", "gate", "change-password"] as const, mutationFn: (input: GatePasswordChangeInput) => gate.changePassword(input), onSuccess: () => invalidate(queryClient, queryKeys.gate.all, queryKeys.capabilities.all) }),
      preferences: {
        update: () => ({
          mutationKey: ["pix", "preferences", "update"] as const,
          mutationFn: (input: PreferencesPatchInput) => preferencesApi.update(input),
        }),
      },
    },
    files: {
    ...
    worktrees: {
      create: () => ({ mutationKey: ["pix", "worktrees", "create"] as const, mutationFn: (input: { cwd: string; branch: string }) => resources.worktrees.create(input), onSuccess: (_data: unknown, input: { cwd: string }) => invalidate(queryClient, queryKeys.worktrees.list(input.cwd), queryKeys.cwd.roots(), queryKeys.projects.all) }),
      remove: () => ({ mutationKey: ["pix", "worktrees", "remove"] as const, mutationFn: (input: { cwd: string; path: string; force?: boolean }) => resources.worktrees.remove(input), onSuccess: (_data: unknown, input: { cwd: string }) => invalidate(queryClient, queryKeys.worktrees.list(input.cwd), queryKeys.cwd.roots(), queryKeys.projects.all) }),
    },
    models: {
      saveConfig: () => ({
        mutationKey: ["pix", "models", "save-config"] as const,
        mutationFn: (input: Parameters<typeof models.saveConfig>[0]) => models.saveConfig(input),
        onSuccess: (data: Awaited<ReturnType<typeof models.saveConfig>>) => {
          queryClient.setQueryData(queryKeys.models.config(), data);
          return invalidate(queryClient, queryKeys.models.list(), queryKeys.auth.all);
        },
      }),
      discover: () => ({
        mutationKey: ["pix", "models", "discover"] as const,
        mutationFn: (input: Parameters<typeof models.discover>[0]) => models.discover(input),
      }),
    },
    settings: {
      ...
      saveConfigFile: () => ({
        ...
        mutationFn: (input: Parameters<typeof configuration.settingsFile.save>[0]) =>
