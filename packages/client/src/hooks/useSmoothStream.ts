...
function defaultFrameIntervalMs(): number {
  ...
  return window.matchMedia("(max-width: 640px)").matches ? 32 : 0;
}

export interface SmoothStreamOptions {
  /** Minimum interval between React publishes. CSS keeps animating between publishes. */
...
export function useSmoothStream(
  ...
  const minFrameIntervalMs = Math.max(0, options.minFrameIntervalMs ?? defaultFrameIntervalMs());
  ...
  const startFrameLoop = useCallback(() => {
    ...
    const tick = (ts: number) => {
      ...
      if (frameIntervalMs < minFrameIntervalMs) {
        rafRef.current = requestAnimationFrame(tick);
        return;
      }
    ...
    rafRef.current = requestAnimationFrame(tick);
  }, [clearWakeTimer, minFrameIntervalMs, scheduleFrameWake, stopFrameLoop]);
