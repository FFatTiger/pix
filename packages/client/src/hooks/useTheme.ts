...
import { fetchResolvedTheme } from "@/api/themes";
...
async function fetchTheme(name: string, mode: ResolvedMode): Promise<ResolvedTheme | null> {
  ...
  // pix adapter: resolution targets /v1/themes/:name?mode= through the api
  // layer (raw fetch is boundary-restricted to src/api). Until the Host themes
  // route lands this returns null and the caller degrades to the default CSS
  // theme — same semantics as the source's failed fetch.
  const resolved = await fetchResolvedTheme(name, mode);
  if (!resolved) return null;
  themeCache.set(cacheKey, resolved);
  return resolved;
}
