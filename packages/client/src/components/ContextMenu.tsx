...
interface ContextMenuApi {
  openMenu: (x: number, y: number, entries: ContextMenuEntry[], options?: { openedByTouch?: boolean }) => void;
  closeMenu: () => void;
}
...
export function ContextMenuProvider({ children }: { children: ReactNode }) {
  ...
  const openMenu = useCallback((x: number, y: number, entries: ContextMenuEntry[], options?: { openedByTouch?: boolean }) => {
    if (feedbackTimerRef.current) clearTimeout(feedbackTimerRef.current);
    feedbackTimerRef.current = null;
    ignoreOutsideUntilRef.current = options?.openedByTouch === true ? Date.now() + 750 : 0;
    setFeedbackIndex(-1);
    setActiveIndex(-1);
    setSubmenuIndex(null);
    setSubmenuActiveIndex(-1);
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: MouseEvent) => {
      if (isInsideMenu(e.target) || Date.now() < ignoreOutsideUntilRef.current) return;
      closeMenu();
    };
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") {
        ...
        if (entry && entry.type !== "separator" && entry.submenu) {
          e.preventDefault();
          setSubmenuIndex(idx);
          setSubmenuActiveIndex(-1);
        }
  ...
  useEffect(() => {
    ...
    if (!parentEntry || parentEntry.type === "separator" || !parentEntry.submenu) return;
  ...
  const menuBody = menu ? (
    ...
    >
      ...
      {submenuIndex !== null && (() => {
        const parentEntry = menu.entries[submenuIndex];
        if (!parentEntry || parentEntry.type === "separator" || !parentEntry.submenu) return null;
