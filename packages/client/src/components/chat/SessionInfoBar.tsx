...
import type { ChatSessionStatsView, ContextUsageView } from "./chat-runtime-view";

export interface SessionInfoBarProps {
  onViewFullHistory?: () => void;
  systemPrompt: string | null;
  sessionStats: ChatSessionStatsView | null;
  ...
  branchTree?: SessionTreeNode[] | undefined;
  branchActiveLeafId?: string | null | undefined;
  onBranchLeafChange?: ((leafId: string | null) => void) | undefined;
...
export function SessionInfoBar({
  ...
  const contextLabel = translate(contextUsage?.estimated
    ? "desktop.sessionInfoEstimatedContext"
    : "desktop.sessionInfoContext");
  ...
  if (contextUsage?.contextWindow && contextUsage.percent !== null) {
    ...
    tooltipParts.push(
      `${contextLabel}: ${pct !== null ? `${pct.toFixed(1)}%` : "?"} ${used !== null ? used.toLocaleString() : "?"}/${contextUsage.contextWindow.toLocaleString()}`,
    );
  }
                ...
                <div className="session-stats-body">
                  {(() => {
                    ...
                    if (ctx?.contextWindow && ctx.percent !== null) {
                      ...
                      tokenRows.push([
                        contextLabel,
                        `${pct !== null ? `${pct.toFixed(1)}%` : "?"} ${used !== null ? used.toLocaleString() : "?"}/${ctx.contextWindow.toLocaleString()}`,
                      ]);
                    }
                        ...
                        <div className="session-stats-side-by-side">
                          ...
                          <div className="session-stats-column">
                            ...
                            <div className="session-stats-compact-grid">
                              ...
                              {(sessionStats.totalActiveMs ?? 0) > 0 && row(translate("desktop.sessionInfoActiveTime"), formatDuration(sessionStats.totalActiveMs ?? 0))}
                            </div>
                          </div>
                        </div>
                      </>
