...
describe("SessionStore — E15 extension-UI incremental input slot", () => {
  ...
  it("detach / stop / dispose settle every queued entry exactly once", async () => {
    // detach
    const h = createHarness();
    const ws = await attach(h, EXT_CAPS);
    const p1 = h.store.sendExtensionUiInput(customRequest, "a");
    ...
    await expect(p1).rejects.toMatchObject({ code: "interrupted", message: "detached" });
    await expect(p2).rejects.toMatchObject({ code: "interrupted", message: "detached" });

    // stop
    const h2 = createHarness();
    const ws2 = await attach(h2, EXT_CAPS);
    const p3 = h2.store.sendExtensionUiInput(customRequest, "a");
    await flush();
    ...
    await expect(p3).rejects.toMatchObject({ code: "interrupted", message: "session stopped" });

    // dispose
    const h3 = createHarness();
    await attach(h3, EXT_CAPS);
    const p4 = h3.store.sendExtensionUiInput(customRequest, "a");
    const p5 = h3.store.sendExtensionUiInput(customRequest, "b");
    await flush();
    h3.store.dispose();
    await expect(p4).rejects.toMatchObject({ code: "unavailable" });
    await expect(p5).rejects.toMatchObject({ code: "unavailable" });
  });

  it("session switch settles the FIFO; a fresh input on the new session is never cross-talked", async () => {
    const h = createHarness();
    const ws = await attach(h, EXT_CAPS, "s1");
    const old1 = h.store.sendExtensionUiInput(customRequest, "a");
    const old2 = h.store.sendExtensionUiInput(customRequest, "b");
    await flush();
    const detachP = h.store.detach();
    await flush();
    const detachFrame = lastFrame<{ type: string; id: string }>(ws, "detach")!;
    ws.serverSend({ type: "response", id: detachFrame.id, payload: { ok: true, result: { sessionId: "s1", detached: true } } });
    await expect(detachP).resolves.toBeUndefined();
    await expect(old1).rejects.toMatchObject({ code: "interrupted", message: "detached" });
    await expect(old2).rejects.toMatchObject({ code: "interrupted", message: "detached" });
    ...
    expect(frame.payload.command.id).toBe("req-input");
    ackInput(ws, frame);
    await expect(next).resolves.toBeUndefined();
  });
