 ...
 * convergence, never silent loss. The WS half lives in {@link RuntimeConnection};
 * this hook covers the HTTP half: when the tab returns
...
import { useRuntimeConnection } from "./runtime-provider.js";
...
export function useResumeRefetch(): void {
  ...
  // Connection-only surface: resume only needs transport state, so it reads the
  // provider-owned connection via useRuntimeConnection. No exact session/lease
  // dependency.
  ...
  }, [connection.state, refetchBootSurface]);
