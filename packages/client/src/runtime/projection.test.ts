...
describe("reduceRuntimeEventData — prompt lifecycle terminals", () => {
  it("keeps the turn running across agent_end retries and settles on agent_settled", () => {
    let snap = reduceRuntimeEventData(base(), { type: "agent_start", sessionId: "s" });
    snap = reduceRuntimeEventData(snap, { type: "agent_end", sessionId: "s" });
    expect(snap.state.isPromptRunning).toBe(true);
    snap = reduceRuntimeEventData(snap, { type: "agent_settled", sessionId: "s" });
    expect(snap.state.isPromptRunning).toBe(false);
  });
  it("prompt_error clears a partial stream even when message_end never arrives", () => {
    let snap = reduceRuntimeEventData(base(), { type: "agent_start", sessionId: "s" });
    snap = reduceRuntimeEventData(snap, { type: "message_start", sessionId: "s", streamId: "st", messageId: "m", message: { role: "assistant", model: "m", provider: "p" } });
    snap = reduceRuntimeEventData(snap, { type: "prompt_error", sessionId: "s", errorMessage: "failed" });
    expect(snap.state.isPromptRunning).toBe(false);
    expect(snap.state.isStreaming).toBe(false);
    expect(snap.streaming).toEqual({ active: false, phase: "idle" });
  });
});
...
describe("reduceRuntimeEventData — compaction / closed / unavailable / crashed settle streaming", () => {
  it("compaction start/end toggles isCompacting", () => {
    ...
    expect(snap.state.compaction?.reason).toBe("auto");
  ...
  it("preserves the automatic reason from dedicated auto-compaction events", () => {
    const snap = reduceRuntimeEventData(base(), { type: "auto_compaction_start", sessionId: "s" });
    expect(snap.state.compaction?.reason).toBe("auto");
  });
