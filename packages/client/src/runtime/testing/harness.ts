 ...
 * and online/visibility hooks so the SessionStore + RuntimeSocket can be driven
...
export function createHarness(options: HarnessOptions = {}): RuntimeHarness {
  ...
  const store = new SessionStore(deps, { ...(options.id === undefined ? {} : { id: options.id }), ...(options.storeOptions ?? {}) });
  return {
    store,
    sockets,
    onlineListeners,
...
export function snapshotPayload(overrides: Partial<{ sessionId: string; epoch: string; lastEventId: number; cwd: string; projectRoot: string; workerStatus: string; resumeStatus: string }> = {}): {
