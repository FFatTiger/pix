...
describe("SessionStore — D2-P8 extension-UI reply slot", () => {
  ...
  it("detach retains an in-flight reply; stop / dispose settle it exactly once", async () => {
    const h = createHarness();
    const ws = await attach(h, EXT_CAPS);
    const p = h.store.respondExtensionUi(confirmRequest, { responseKind: "confirmed", confirmed: true });
    await flush();
    const detachedReply = extFrame(ws);
    ...
    expect(h.controller("s1")?.evictionProtection.pendingExtensionResponse).toBe(true);
    respondOk(ws, detachedReply.id, detachedReply.payload.command.commandId!);
    await expect(p).resolves.toBeUndefined();
  ...
  it("session switch retains the A reply and B owns an independent extension response lane", async () => {
    const h = createHarness();
    ...
    expect(h.controller("s1")?.evictionProtection.pendingExtensionResponse).toBe(true);
    expect(h.controller("s2")?.evictionProtection.pendingExtensionResponse).toBe(true);
    respondOk(ws, frame1.id, frame1.payload.command.commandId!);
    await expect(p).resolves.toBeUndefined();
    respondOk(ws, frame2.id, frame2.payload.command.commandId!);
    await expect(next).resolves.toBeUndefined();
  });
