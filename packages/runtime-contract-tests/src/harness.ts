...
export interface HarnessFactoryOptions {
  ...
  /**
   * Deterministic contract knob: the NEXT authoritative snapshot capture fails
   * (one-shot) so a submitTurn admission-snapshot failure is proven to yield a
   * rejected/not-delivered admission with ZERO prompt launch.
   */
