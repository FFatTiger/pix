...
export function createRuntimeAdapterSuite(harness: AdapterContractHarness): void {
  describe("agent runtime adapter contract", () => {
    ...
    describe("capability & command", () => {
      ...
      it("independent get_state read returns the canonical runtime state", async () => {
        const { port } = await newRuntime(harness);
        const result = await port.read({ type: "get_state" });
      ...
      it("independent reads return typed payloads", async () => {
        const { port } = await newRuntime(harness);
        await port.execute({ type: "prompt", message: "hi" });

        const tools = await port.read({ type: "get_tools" });
        ...
        const commands = await port.read({ type: "get_commands" });
        ...
        const stats = await port.read({ type: "get_session_stats" });
        assert.equal(stats.ok, true);
        if (stats.ok && stats.type === "get_session_stats") {
          assert.equal(stats.stats.messageCount, 3);
        }

        const last = await port.read({ type: "get_last_assistant_text" });
      ...
      it("submitTurn admits quickly while the prompt runs deferred; reads stay independent", async () => {
        const { port } = await newRuntime(harness);
        const handle = await port.submitTurn({ prompt: "long prompt" });
        assert.equal(handle.admission.ok, true, "admission must be quick (before the prompt completes)");
        await waitForSnapshot(port, (snapshot) => snapshot.state.isPromptRunning);
        // Reads stay available on the independent lane while the turn runs.
        const commands = await port.read({ type: "get_commands" });
        assert.equal(commands.ok, true);
        const state = await port.read({ type: "get_state" });
        assert.equal(state.ok, true);
        // A second submitTurn while the turn runs is session_busy.
        const second = await port.submitTurn({ prompt: "another" });
        assert.equal(second.admission.ok, false);
        if (!second.admission.ok) assert.equal(second.admission.error.code, "session_busy");
        const terminal = await handle.completion;
        assert.equal(terminal.ok, true);
        await port.close("user");
      });

      it("submitTurn override failure admits false with zero prompt launch (no rollback fiction)", async () => {
        const { port } = await newRuntime(harness);
        const handle = await port.submitTurn({ prompt: "hi", activationOverrides: { thinkingLevel: "bogus" as never } });
        assert.equal(handle.admission.ok, false);
        if (!handle.admission.ok) assert.equal(handle.admission.error.code, "invalid_input");
        const terminal = await handle.completion;
        assert.equal(terminal.ok, false);
        // The prompt never launched: no running state, no user entry appended.
        const snapshot = await port.getSnapshot();
        assert.equal(snapshot.state.isPromptRunning, false);
        await port.close("user");
      });

      it("submitTurn with a valid model override admits, terminals ok, and the assistant actually ran on the override model", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        const override = { provider: "openai", modelId: "gpt-5" };
        // Guard against a vacuous assertion: the override must differ from the
        // runtime's current (default) model so a pass proves an actual change.
        const before = await port.getSnapshot();
        assert.ok(
          before.state.model?.provider !== override.provider || before.state.model?.id !== override.modelId,
          "the override must differ from the current model for the assertion to be meaningful",
        );
        const handle = await port.submitTurn({ prompt: "hi", activationOverrides: { model: override } });
        assert.equal(handle.admission.ok, true);
        if (handle.admission.ok) {
          assert.equal(handle.admission.snapshot.state.model?.provider, override.provider);
          assert.equal(handle.admission.snapshot.state.model?.id, override.modelId);
        }
        const terminal = await handle.completion;
        assert.equal(terminal.ok, true);
        assert.equal(terminal.snapshot.state.model?.provider, override.provider);
        assert.equal(terminal.snapshot.state.model?.id, override.modelId);
        // The committed assistant message carries the model the turn ACTUALLY
        // ran on — never a successful terminal under a different model.
        const assistantEnds = collector.ofType("message_end").filter(
          (event) => (event.message as { role?: string }).role === "assistant",
        );
        assert.ok(assistantEnds.length > 0, "the turn must commit an assistant message");
        const final = assistantEnds[assistantEnds.length - 1]!;
        assert.equal((final.message as { model?: string }).model, override.modelId);
        assert.equal((final.message as { provider?: string }).provider, override.provider);
        collector.dispose();
        await port.close("user");
      });

      it("submitTurn admission snapshot failure admits false with zero prompt launch (error honesty)", async () => {
        const { port } = await newRuntime(harness, { failAdmissionSnapshot: true });
        const handle = await port.submitTurn({ prompt: "hi" });
        // The authoritative admission snapshot failed BEFORE the prompt could be
        // launched: the admission must report a truthful rejection, never a
        // clean acceptance with a prompt continuing in the background.
        assert.equal(handle.admission.ok, false);
        if (!handle.admission.ok) assert.equal(handle.admission.error.code, "unavailable");
        const terminal = await handle.completion;
        assert.equal(terminal.ok, false);
        // Zero prompt launch: the runtime is idle again and a fresh submitTurn is
        // admitted — a leaked prompt would reject it as session_busy.
        const again = await port.submitTurn({ prompt: "now admitted" });
        assert.equal(again.admission.ok, true, "a failed admission snapshot must leave the runtime idle (zero prompt)");
        const terminalAgain = await again.completion;
        assert.equal(terminalAgain.ok, true);
        const snapshot = await port.getSnapshot();
        assert.equal(snapshot.state.isPromptRunning, false);
        await port.close("user");
      });
    ...
    describe("compaction", () => {
      ...
      it("auto compaction exposes recoverable state while running", async () => {
        ...
        // The prompt starts one microtask after the (now snapshot-first)
        // admission barrier, so the running-compaction window opens just after
        // execute() returns. Poll tightly (1ms) so the short auto-compact
        // window is observed deterministically instead of being missed by a
        // coarse 10ms poll.
        const snapshot = await waitForSnapshot(
          port,
    ...
    describe("catalog & locator (requires createPorts)", () => {
      ...
      it("defers persisted thinking and resolves it by exact session/entry/block identity", async () => {
        const factory = await harness.createFactory();
        const port = await factory.create({ cwd: "/workspace" });
        const thinking = await port.execute({ type: "set_thinking_level", level: "high" });
        assert.equal(thinking.ok, true);
        await port.execute({ type: "prompt", message: "explain the plan" });
        const ports = await harness.createPorts!(factory);
        const context = await ports.sessionCatalog.readSessionContext(port.identity.sessionId, {
          deferThinking: true,
          deferMedia: true,
        });
        const assistant = context.entries.find((entry) =>
          entry.message.role === "assistant"
          && entry.message.content.some((block) => block.type === "thinking" && block.deferred === true),
        );
        assert.ok(assistant, "deferred context must retain an exact thinking placeholder");
        if (assistant?.message.role !== "assistant") throw new Error("unreachable");
        const blockIndex = assistant.message.content.findIndex(
          (block) => block.type === "thinking" && block.deferred === true,
        );
        assert.ok(blockIndex >= 0);
        const resolved = await ports.sessionCatalog.readSessionThinking(
          port.identity.sessionId,
          assistant.entryId,
          blockIndex,
        );
        assert.equal(resolved.sessionId, port.identity.sessionId);
        assert.equal(resolved.entryId, assistant.entryId);
        assert.equal(resolved.blockIndex, blockIndex);
        assert.ok(resolved.thinking.trim().length > 0, "persisted thinking is restored verbatim");
        await assert.rejects(() =>
          ports.sessionCatalog.readSessionThinking(port.identity.sessionId, assistant.entryId, 9999),
        );
        await port.close("user");
      });
    ...
    /* ---------------------------------------------------------------- */
    describe("session_changed events (Phase 5A end-to-end bridge)", () => {
      /**
       * The canonical runtime-core `session_changed{cwd, leafId}` RuntimeEvent
       * (mapped by the worker onto the existing Protocol
       * `SessionChangedEventData` frame) is emitted on the REGULAR
       * `AgentRuntimePort.subscribe` channel ONLY after a successful navigate
       * and at compaction terminal — never for a rejected command, and never
       * guessed (cwd/leafId come from the authoritative adapter state). The
       * shared suite proves it for EVERY adapter through the ordinary event
       * channel; adapters do not duplicate this contract in private tests.
       */
      it("successful navigate emits exactly one session_changed carrying the navigated leaf + cwd", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        try {
          // Build a deterministic multi-entry branch so an EARLIER leaf exists.
          await port.execute({ type: "prompt", message: "one" });
          const snapshot = await port.getSnapshot();
          assert.ok(snapshot.state.leafId, "prompted session must expose a committed leaf");
          const target = "entry-1";
          const result = await port.execute({ type: "navigate_tree", targetId: target });
          assert.equal(result.ok, true, "navigate must succeed");
          const changes = collector.ofType("session_changed");
          assert.equal(changes.length, 1, "exactly one session_changed after a successful navigate");
          assert.equal(changes[0]?.sessionId, port.identity.sessionId);
          assert.equal(typeof changes[0]?.cwd, "string");
          assert.ok((changes[0]?.cwd ?? "").length > 0, "cwd is the authoritative session cwd");
          assert.equal(changes[0]?.leafId, target, "the event carries the NAVIGATED leaf");
        } finally {
          collector.dispose();
        }
        await port.close("user");
      ...
      it("unsubscribing stops delivery (no leak, no replay)", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        await port.execute({ type: "prompt", message: "one" });
        collector.dispose();
        await port.execute({ type: "navigate_tree", targetId: "entry-1" });
        assert.equal(collector.ofType("session_changed").length, 0, "unsubscribed listener never receives events");
        await port.close("user");
      });

      it("rejected navigate (blank target) emits NO session_changed", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        try {
          // Blank target is invalid_input on EVERY conforming adapter (the
          // canonical boundary fails closed before any backend mutation).
          const invalid = await port.execute({ type: "navigate_tree", targetId: "   " });
          assert.equal(invalid.ok, false);
        } finally {
          collector.dispose();
        }
        assert.equal(collector.ofType("session_changed").length, 0, "a rejected navigate never reports a session change");
        await port.close("user");
      });

      it("manual compaction terminal emits exactly one session_changed with the authoritative post-compaction leaf", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        try {
          await port.execute({ type: "prompt", message: "hi" });
          ...
          assert.equal(end.type, "compaction_end");
          const changes = collector.ofType("session_changed");
          assert.equal(changes.length, 1, "exactly one session_changed at compaction terminal");
          assert.equal(changes[0]?.sessionId, port.identity.sessionId);
          assert.ok((changes[0]?.cwd ?? "").length > 0);
          const snapshot = await port.getSnapshot();
          // The event leaf is the authoritative post-compaction leaf (or omitted
          // when the backend exposes none) — never a guessed value.
          if (snapshot.state.leafId !== undefined) {
            assert.equal(changes[0]?.leafId, snapshot.state.leafId);
          }
        } finally {
          collector.dispose();
        }
      ...
      it("auto compaction terminal also emits session_changed", async () => {
        const { port } = await newRuntime(harness);
        const collector = new EventCollector(port);
        try {
          await port.execute({ type: "set_auto_compaction", enabled: true });
          const pending = port.execute({ type: "prompt", message: "auto-compact now" });
          await collector.waitFor((event) => event.type === "auto_compaction_end", 10_000);
          await pending;
          assert.equal(collector.ofType("session_changed").length, 1, "auto compaction terminal publishes the session change");
        } finally {
          collector.dispose();
        }
        await port.close("user");
