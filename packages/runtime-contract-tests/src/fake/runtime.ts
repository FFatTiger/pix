...
export interface ReferenceRuntimeOptions {
  ...
  /**
   * Deterministic contract knob: the NEXT snapshot capture fails with
   * `unavailable` (one-shot) so a submitTurn admission snapshot failure can be
   * proven to yield a rejected/not-delivered admission with ZERO prompt launch.
   */
  failAdmissionSnapshot?: boolean;
}
...
export class ReferenceAgentRuntime implements AgentRuntimePort {
  ...
  /** Honest "turn admitted but prompt not yet launched" marker used to build the
   * admission snapshot BEFORE driver.prompt() starts (mirrors the adapter). */
  private admissionRunning = false;
  private failAdmissionSnapshot = false;
  ...
  constructor(options: ReferenceRuntimeOptions) {
    ...
    this.failAdmissionSnapshot = options.failAdmissionSnapshot ?? false;
    this.identity = {
  ...
  async getSnapshot(): Promise<RuntimeSnapshot> {
    ...
    if (this.failAdmissionSnapshot) {
      this.failAdmissionSnapshot = false;
      throw makeRuntimeError("unavailable", "admission snapshot failed");
    }
    ...
    return {
      ...
      streaming:
        this.status === "prompt" || this.admissionRunning
          ? {
              active: true,
  ...
  /**
   * Phase 5A: publish the canonical runtime-core `session_changed{cwd, leafId}`
   * event on the REGULAR subscribe channel (mirrors the production adapter
   * contract — the shared suite consumes it like any other runtime event).
   * Emitted after a successful `navigate_tree` and at compaction terminal only.
   */
  private emitSessionChanged(): void {
    if (this.closed) return;
    const leafId = this.store.getSession(this.identity.sessionId)?.leafId;
    this.emit({
  ...
  async submitTurn(input: RuntimeTurnStart): Promise<RuntimeTurnHandle> {
    const reject = async (error: RuntimeError): Promise<RuntimeTurnHandle> => {
      const snapshot = await this.getSnapshot();
      return { admission: { ok: false, error, snapshot }, completion: Promise.resolve({ ok: false, error, snapshot }) };
    };
    if (this.closed) return reject(makeRuntimeError("unavailable", "runtime is closed"));
    if (this.status !== "idle") return reject(makeRuntimeError("session_busy", "a turn is already running", { retryable: true }));
    const beforeCount = this.store.getSession(this.identity.sessionId)?.entries.length ?? 0;
    if (input.activationOverrides?.model !== undefined) {
      const result = await this.executeSetModel({ type: "set_model", provider: input.activationOverrides.model.provider, modelId: input.activationOverrides.model.modelId });
      if (!result.ok) return reject(result.error);
    }
    if (input.activationOverrides?.thinkingLevel !== undefined) {
      if (!THINKING_LEVELS.includes(input.activationOverrides.thinkingLevel)) return reject(makeRuntimeError("invalid_input", "thinking level is invalid"));
      this.thinkingLevel = input.activationOverrides.thinkingLevel;
      this.thinkingLevelPinned = true;
      this.emitStateChanged();
    }
    // The admission barrier completes BEFORE the prompt is launched: mark the
    // running state honestly, then capture the authoritative admission snapshot.
    // If the snapshot fails (contract knob), NO prompt call has happened and the
    // admission reports a clean rejected/not-delivered outcome — nothing runs in
    // the background.
    this.admissionRunning = true;
    let snapshot: RuntimeSnapshot;
    try {
      snapshot = await this.getSnapshot();
    } catch {
      this.admissionRunning = false;
      return reject(makeRuntimeError("unavailable", "admission snapshot failed"));
    }
    const execution = this.executePrompt({ type: "prompt", message: input.prompt, ...(input.images === undefined ? {} : { images: input.images }) });
    this.admissionRunning = false; // runTurn flips status to "prompt" synchronously
    const completion = execution.then(async (result) => {
      const terminalSnapshot = await this.getSnapshot();
      const userEntryId = this.store.getSession(this.identity.sessionId)?.entries.slice(beforeCount).find((entry) => entry.message.role === "user")?.entryId;
      return result.ok
        ? { ok: true as const, snapshot: terminalSnapshot, ...(userEntryId === undefined ? {} : { userEntryId }) }
        : { ok: false as const, error: result.error, snapshot: terminalSnapshot, ...(userEntryId === undefined ? {} : { userEntryId }) };
    });
  ...
  async execute(command: RuntimeCommand): Promise<RuntimeCommandResult> {
    ...
    switch (command.type) {
      ...
      case "navigate_tree": {
        ...
        // Phase 5A: successful navigate — publish the moved branch pointer.
  ...
  /**
   * Phase 2B read lane: pure reads over the reference projection. Reads never
   * mutate state and stay available while a prompt/bash/compaction is pending.
   */
    ...
    switch (request.type) {
      ...
      case "get_session_stats": {
        ...
        return {
          ...
          stats: {
            messageCount: session.entries.length,
            pendingMessageCount: this.queue.steering.length + this.queue.followUp.length,
            tokenCount: session.entries.length * 100,
            contextUsage: {
              percent: Math.min(100, session.entries.length * 10),
              tokens: session.entries.length * 1000,
  ...
  private buildState(): RuntimeState {
    ...
    return {
      ...
      isStreaming: this.status === "prompt" || this.admissionRunning,
      isPromptRunning: this.status === "prompt" || this.admissionRunning,
