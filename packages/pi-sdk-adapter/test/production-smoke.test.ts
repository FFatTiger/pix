  ...
  it("production surface returns unsupported_capability for commands outside the set", async () => {
    await withAgentDir(async (root) => {
      ...
      try {
        ...
        // gate must report it OPEN. Real title generation performs a model
        // call through a temporary shadow agent; on a fresh session with no
        // user messages it must fail closed with a structured sanitized error
        // BEFORE any model request (deterministic offline), proving the command
        // reached the adapter (never unsupported_capability, which would prove
        ...
        assert.equal(autoName.ok, false, "fresh-session auto_name must not claim success without user messages");
        if (!autoName.ok) {
          assert.equal(autoName.error.code, "external", "open gate must reach the adapter (not unsupported_capability)");
          assert.ok(!/\n|\tat |node:internal/i.test(autoName.error.message), "no raw stack text");
        }
