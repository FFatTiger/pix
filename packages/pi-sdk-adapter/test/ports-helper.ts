...
export function createPortsFromBackend(backend: PiSdkDataBackend): {
  ...
  return {
    sessionCatalog: {
      ...
      readSessionContext: (id, options) => backend.readSessionContext(id, {
      ...
      readSessionThinking: (id, entryId, blockIndex) => backend.readSessionThinking(id, entryId, blockIndex),
