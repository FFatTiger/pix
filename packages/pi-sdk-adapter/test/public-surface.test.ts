...
describe("public agent factory surface", () => {
  ...
  it("PRODUCTION_AGENT_CAPABILITIES is exactly prompt+abort+stats+rename+thinking.set+model.set and leaks no broader surface", () => {
    ...
    // Still-forbidden: tools/bash/fork/extension-UI/queue/auto_name/reload
    // must NOT leak through the production surface. `runtime.stats` +
    // `runtime.session.rename` (D2-P1), `runtime.thinking.set` (D2-P2) and
    // `runtime.model.set` (D2-P3) ARE allowed here.
    const leaked = [...PRODUCTION_AGENT_CAPABILITIES].filter((capability) =>
      /tools|bash|fork|extension_ui|navigate|compact|reload|queue|auto_name/.test(capability),
    );
    assert.deepEqual(leaked, [], "production surface must not leak tools/bash/fork/extension-UI/queue/auto_name/reload capabilities");
  });
});
