...
export class ScriptedSdkStore implements PiSdkDataBackend {
  ...
  async readSessionContext(id: string, options: { leafId?: string } = {}): Promise<SessionContext> { const s = this.need(id); const entries = structuredClone(s.entries); return { sessionId: id, ...(options.leafId ?? s.leaf ? { leafId: options.leafId ?? s.leaf! } : {}), entries, pageInfo: { hasMore: false } }; }
