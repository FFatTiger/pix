...
export class ScriptedSdkStore implements PiSdkDataBackend {
  ...
  async readSessionContext(
    ...
    const projected = structuredClone(s.entries).map((entry) => {
      if (entry.message.role !== "assistant" || options.deferThinking !== true) return entry;
      return {
        ...entry,
        message: {
          ...entry.message,
    ...
    const beforeIndex = options.before === undefined
      ? projected.length
      : projected.findIndex((entry) => entry.entryId === options.before);
    if (beforeIndex < 0) throw makeRuntimeError("invalid_input", "history cursor is invalid");
    const start = options.limit === undefined ? 0 : Math.max(0, beforeIndex - options.limit);
    const entries = projected.slice(start, beforeIndex);
    return {
      sessionId: id,
      ...(options.leafId ?? s.leaf ? { leafId: options.leafId ?? s.leaf! } : {}),
      entries,
      pageInfo: {
        hasMore: start > 0,
        ...(start > 0 && entries[0] !== undefined ? { nextCursor: entries[0].entryId } : {}),
      },
    };
  }
  async readSessionThinking(id: string, entryId: string, blockIndex: number): Promise<SessionThinkingBlock> {
    const s = this.need(id);
    const entry = s.entries.find((candidate) => candidate.entryId === entryId);
    if (!entry || entry.message.role !== "assistant") throw makeRuntimeError("not_found", "entry not found");
    const block = entry.message.content[blockIndex];
    if (!block || block.type !== "thinking") throw makeRuntimeError("not_found", "entry not found");
    return { sessionId: id, entryId, blockIndex, thinking: block.thinking };
  }
...
export class ScriptedSdkDriverFactory implements PiRuntimeDriverFactory {
  constructor(
    ...
    private readonly defaults?: { cwd?: string; model?: { provider: string; modelId: string }; failGetStateOnCall?: number },
  ) {}
  async create(input: RuntimeStartInput, options: DriverFactoryOptions) {
    return new ScriptedSdkDriver(
      ...
      options,
      this.defaults?.failGetStateOnCall,
    );
  }
  async open(id: string, _cwd: string | undefined, model: ModelRef | undefined, options: DriverFactoryOptions) { const s = this.store.need(id); if (model) s.model = model; return new ScriptedSdkDriver(this.store, s, { cwd: s.cwd }, options, this.defaults?.failGetStateOnCall); }
}
...
class ScriptedSdkDriver implements PiRuntimeDriver {
  ...
  constructor(private store: ScriptedSdkStore, private session: Stored, input: RuntimeStartInput, private opts: DriverFactoryOptions, private readonly failGetStateOnCall?: number) {
  ...
  getState(): DriverState {
    ...
    if (this.failGetStateOnCall !== undefined && this.getStateCalls === this.failGetStateOnCall) {
      throw makeRuntimeError("unavailable", "admission snapshot failed");
    }
    return { model: this.model, thinkingLevel: this.thinking, systemPrompt: this.systemPrompt, isStreaming: this.status === "prompt", isCompacting: this.status === "compact", isBashRunning: this.status === "bash", autoCompactionEnabled: this.autoCompact, autoRetryEnabled: this.autoRetry, pendingMessageCount: this.steering.length + this.follow.length, ...(this.session.leaf === undefined ? {} : { leafId: this.session.leaf }), messageCount: this.session.entries.length, tools: [...this.tools].map(([name, active]) => ({ name, active })), contextUsage: { percent: this.session.entries.length ? Math.min(100, this.session.entries.length * 10) : 0, tokens: this.session.entries.length * 1000, contextWindow: 200000 }, steering: structuredClone(this.steering), followUp: structuredClone(this.follow), sessionStats: { messageCount: this.session.entries.length, pendingMessageCount: this.steering.length + this.follow.length, tokenCount: this.session.entries.length * 100, contextUsage: { percent: this.session.entries.length * 10, tokens: this.session.entries.length * 1000, contextWindow: 200000 } }, lastAssistantText: this.last, commands: this.store.commands, ...(this.session.title === undefined ? {} : { sessionName: this.session.title }) }; }
  getTools(): readonly ToolInfo[] { return [...this.tools].map(([name, active]) => ({ name, active })); }
  getCommands(): readonly SlashCommandInfo[] { return this.store.commands; }
  getSessionStats(): DriverState["sessionStats"] { return { messageCount: this.session.entries.length, pendingMessageCount: this.steering.length + this.follow.length, tokenCount: this.session.entries.length * 100, contextUsage: { percent: this.session.entries.length * 10, tokens: this.session.entries.length * 1000, contextWindow: 200000 } }; }
  getLastAssistantText(): string { return this.last; }
  resolveLeafEntry(_expectedRole?: string) { const entries = this.session.entries; const leaf = this.session.leaf; if (!leaf) return undefined; const entry = entries.find((e) => e.entryId === leaf); if (!entry) return undefined; return { entryId: entry.entryId, ...(entry.parentEntryId === undefined ? {} : { parentEntryId: entry.parentEntryId }) }; }
