...
async function seedAgentDir(): Promise<string> {
  ...
  // models.json decides which providers are LISTED (config-file-only scope).
  // anthropic is configured here; openai is NOT — its stored OAuth credential
  // must not add a catalog row.
  await writeFile(
    join(agentDir, "models.json"),
    JSON.stringify({
      providers: {
        anthropic: { apiKey: SECRETS.apiKey },
      },
    }),
    "utf8",
  );
  await writeFile(
    ...
    JSON.stringify({
      // Stored API-key provider (also models.json-listed) — authorized without leaking the key.
      anthropic: { type: "api_key", key: SECRETS.apiKey },
      // Stored OAuth provider NOT in models.json — never listed; status-by-id
      // still works; refresh/access must never cross the boundary.
      openai: {
        type: "oauth",
...
describe("read-only credential catalog (D3B-R1A)", () => {
  it("listProviders returns ONLY models.json-configured providers, sanitized, offline", async () => {
    ...
    try {
      ...
      release();
      // Config-file scope: exactly the models.json provider ids.
      assert.deepEqual(
        providers.map((provider) => provider.id).sort(),
        ["anthropic"],
        "stored-credential-only providers (openai) must stay unlisted",
      );
  ...
  it("OAuth provider status by id never leaks refresh/access tokens", async () => {
    const root = await seedAgentDir();
    try {
      const catalog = createPiSdkCredentialCatalog({ agentDir: join(root, "agent") });
      // openai is NOT models.json-listed, but a direct status lookup by id
      // still answers truthfully (and never exposes tokens).
      const status = await catalog.getProviderStatus("openai");
      const serialized = JSON.stringify(status);
      assertNoSecrets("oauth-status", serialized);
      // authorized reflects stored OAuth without exposing tokens.
      assert.equal(status.authorized, true);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("missing providers or blank provider identity rejects (not fake empty)", async () => {
    ...
    for (const payload of invalidFiles) {
      const root = await mkdtemp(join(tmpdir(), "pix-credentials-invalid-provider-id-"));
      const agentDir = join(root, "agent");
      await mkdir(agentDir, { recursive: true });
  ...
  it("schema-invalid models.json rejects listing with fixed invalid_input", async () => {
    const root = await mkdtemp(join(tmpdir(), "pix-credentials-badschema-"));
    const agentDir = join(root, "agent");
    await mkdir(agentDir, { recursive: true });
    await writeFile(
      join(agentDir, "models.json"),
      JSON.stringify({ providers: { anthropic: { models: [{ id: 123 }] } } }),
      "utf8",
    );
    try {
      const catalog = createPiSdkCredentialCatalog({ agentDir });
      await assert.rejects(
        ...
        (error: unknown) => {
          ...
          assertNoSecrets("bad-schema-error", String(error));
          return true;
        },
      );
    } finally {
...
describe("credential status consistency + zero-write (D3B-R1A hardening)", () => {
  ...
  it("absent provider (no stored, no env, not in models.json) reports not configured", async () => {
    const root = await seedAgentDir();
    try {
      await withEnv({}, async () => {
        const catalog = createPiSdkCredentialCatalog({ agentDir: join(root, "agent") });
        const configured = await catalog.isConfigured("groq");
        const status = await catalog.getProviderStatus("groq");
