 ...
 * `runtime.thinking.set` (set_thinking_level). Model switching, tool
 * configuration, bash, fork, extension UI, compaction, navigation, reload,
 * the queue/retry knobs and `runtime.auto_name` (generate_session_title) are
 * all implemented by the adapter but gated out by this set so composition
 * (R1/R2) must explicitly opt into broader capabilities. The constant is
 * intentionally narrow and MUST NOT grow to leak
 * model/tools/bash/fork/extension-UI/queue/auto_name/reload capabilities by
 * default.
 */
