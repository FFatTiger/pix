 ...
 * the single real extension-UI slice. D2 fork precisely adds `runtime.fork`
 * (fork) — the single real session-fork slice (backend-first; no Client UI;
 * §57 navigate is the parallel in-flight slice that merges its
 * `runtime.navigate` token into this shared list). The existing
 ...
 * deliberately adds NO Client helper/UI for auto compaction. Navigation and
 * `runtime.auto_name`
 ...
 * leak navigate/auto_name capabilities by default.
 */
