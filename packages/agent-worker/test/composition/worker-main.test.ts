...
describe("worker-main composition (in-process)", () => {
  it("drives init → ready → command → shutdown through real transport/controller wiring", async () => {
    ...
    writeFrame(stdin, { type: "worker.init", id: "init-1", protocolVersion: 2, payload: { mode: "create", sessionId: "provisional", epoch: "e1", cwd: "/workspace", projectRoot: "/workspace", build: SESSIOND_BUILD_IDENTITY } });
    await tick();
    await tick();
    assert.ok(frames.some((f) => f.type === "worker.sessionDiscovered"));
    assert.ok(frames.some((f) => f.type === "worker.ready"));

    writeFrame(stdin, { type: "worker.command", id: "wire-1", protocolVersion: 2, payload: { sessionId: "sess-real", epoch: "e1", command: { commandId: "cmd-1", type: "prompt", message: "hi" } } });
    await tick();
    await tick();
    await tick();
...
describe("worker-main composition (real child process)", () => {
  ...
  it("spawns the executable, completes create/rekey/prompt/shutdown, and exits cleanly on EOF", async () => {
    ...
    writeFrame(child.stdin, {
      ...
      id: "init-1",
      protocolVersion: 2,
      payload: { mode: "create", sessionId: "provisional", epoch: "e1", cwd: "/workspace", projectRoot: "/workspace", build: SESSIOND_BUILD_IDENTITY },
    });
    ...
    writeFrame(child.stdin, {
      ...
      id: "wire-1",
      protocolVersion: 2,
      payload: { sessionId: "sess-created-real", epoch: "e1", command: { commandId: "cmd-1", type: "prompt", message: "hi" } },
    });
