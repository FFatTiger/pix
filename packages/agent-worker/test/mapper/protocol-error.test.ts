...
describe("protocol-error sanitization", () => {
  it("redacts secrets, bearer tokens, api keys and absolute paths", () => {
    const text = redactText(
      "token=sk-abc123XYZ /Users/proxy/Documents/secret/file.txt api_key=123456 Bearer eyJhbGciOi.eyJzdWI.Ci",
    );
    assert.equal(text.includes("sk-abc123XYZ"), false);
    assert.equal(text.includes("/Users/proxy/Documents"), false);
  ...
  it("maps thrown backend errors to a sanitized internal error", () => {
    const error = toProtocolError(new Error("boom at /Users/proxy/x"));
    assert.equal(error.code, "internal");
    assert.equal(error.retryable, false);
    assert.equal(error.message.includes("/Users/proxy"), false);
  });
