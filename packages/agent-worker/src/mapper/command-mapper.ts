 ...
 */
import type { RuntimeCommand as ProtocolRuntimeCommand, RuntimeReadOutcome as ProtocolRuntimeReadOutcome, RuntimeReadRequest as ProtocolRuntimeReadRequest } from "@fffattiger/pix-protocol";
import type {
  RuntimeCommand as CoreRuntimeCommand,
  RuntimeCommandResult as CoreRuntimeCommandResult,
  RuntimeReadResult as CoreRuntimeReadResult,
...
/**
 * Map a Protocol read request onto the canonical Core read request. Reads have
 * a single `type` discriminant, so this is a straight structural pass-through
 * (no wire correlation id — correlation is the Protocol lane's concern).
 */
export function mapProtocolReadToCoreRead(read: ProtocolRuntimeReadRequest): CoreRuntimeReadRequest {
  return { type: read.type };
}

/**
 * Map a Core {@link RuntimeReadResult} back to the Protocol read outcome. The
 * payloads are rebuilt into Protocol DTOs (same mapping helpers as the command
 * path); errors are sanitized.
 */
export function mapCoreReadToProtocolRead(result: CoreRuntimeReadResult): ProtocolRuntimeReadOutcome {
  if (!result.ok) {
    return {
      ok: false as const,
      type: result.type,
