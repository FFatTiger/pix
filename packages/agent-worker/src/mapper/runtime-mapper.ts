  ...
  mapEvent(event: RuntimeEvent): RuntimeEventData[] {
    ...
    switch (event.type) {
      ...
      case "session_changed":
        // Phase 5A canonical session-change event → the EXISTING Protocol
        // `SessionChangedEventData` frame (cwd/sessionFile?/leafId?). The
        // adapter guarantees a non-empty cwd and an authoritative leaf (or
        // none); sessiond stamps the wire cursor on the generic worker.event
        // lane exactly like every other event. Field-by-field rebuild (never a
        // spread/cast) so the frozen transport zod gate stays the runtime net.
        return [
          {
            type: "session_changed" as const,
            sessionId,
            cwd: event.cwd,
            ...(event.leafId === undefined ? {} : { leafId: event.leafId }),
            ...ts(event),
          },
        ];
      case "runtime_state_changed":
        // Context-usage consistency: forward the optional atomic
        // {model, leafId, contextUsage} payload when the adapter captured one;
        // a payload-less event stays signal-only. Field-by-field rebuild (never
        // a spread/cast) so the frozen transport zod gate stays the runtime net.
        return [
          {
            type: "runtime_state_changed" as const,
            sessionId,
            ...(event.context === undefined
              ? {}
              : {
                  context: {
                    model: mapModelRef(event.context.model),
                    leafId: event.context.leafId,
                    contextUsage: event.context.contextUsage === null
                      ? null
                      : {
                          percent: event.context.contextUsage.percent,
                          ...(event.context.contextUsage.contextWindow === undefined
                            ? {}
                            : { contextWindow: event.context.contextUsage.contextWindow }),
                          ...(event.context.contextUsage.tokens === undefined
                            ? {}
                            : { tokens: event.context.contextUsage.tokens }),
                        },
                  },
                }),
            ...ts(event),
