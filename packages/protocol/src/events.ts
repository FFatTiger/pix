...
/**
 * Coherent context payload for `runtime_state_changed` (context-usage
 * consistency). Wire mirror of Runtime Core `RuntimeStateChangedContext`:
 * {model, leafId, contextUsage} captured from ONE backend read and applied
 * atomically by the shared reducer. `contextUsage: null` is the honest UNKNOWN
 * and clears a stale projected value; `leafId: null` clears the branch pointer.
 */
export const RuntimeStateChangedContextSchema = z.strictObject({
  model: ModelRefSchema.nullable(),
  leafId: NonEmptyStringSchema.nullable(),
  contextUsage: ContextUsageSchema.nullable(),
});
export type RuntimeStateChangedContext = z.infer<typeof RuntimeStateChangedContextSchema>;

/**
 * `runtime_state_changed` keeps its signal-only semantics for consumers that
 * only need "re-read the snapshot"; the optional `context` payload lets
 * payload-aware consumers update model/leaf/usage atomically without a
 * snapshot round-trip (no polling, no snapshot storms). Additive and optional
 * on purpose: an older producer stays signal-only.
 */
export const RuntimeStateChangedEventDataSchema = z.strictObject({
  ...eventDataBase,
  type: z.literal("runtime_state_changed"),
  context: RuntimeStateChangedContextSchema.optional(),
});
