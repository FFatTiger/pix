...
function applyEventToSnapshot(snapshot: RuntimeSnapshot, event: RuntimeEventData): void {
  ...
  switch (event.type) {
    ...
    case "auto_compaction_start":
      ...
      state.compaction = {
        reason: event.type === "auto_compaction_start" || event.reason === "auto" ? "auto" : "manual",
        status: "running",
    ...
    case "runtime_state_changed":
      // Context-usage consistency: the optional atomic {model, leafId,
      // contextUsage} payload updates ALL THREE fields together — captured from
      // one backend read, so a model switch can never land next to a stale
      // usage. `contextUsage: null` (unknown) and `leafId: null` (no entries)
      // CLEAR previously projected values; a payload-less event stays the
      // signal-only "re-read the snapshot" semantic and mutates nothing.
      if (event.context !== undefined) {
        state.model = clone(event.context.model);
        if (event.context.leafId === null) delete state.leafId;
        else state.leafId = event.context.leafId;
        state.contextUsage = clone(event.context.contextUsage);
      }
      break;
    ...
    // `agent_end` is NOT a turn terminal: the Pi SDK emits it for each model
    // attempt, including attempts followed by an automatic retry. Clearing here
    // makes global busy state and the Composer flicker idle between retries.
    // Only the settled/prompt terminal events end the full prompt lifecycle.
