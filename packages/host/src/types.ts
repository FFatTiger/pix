...
import type { GatePreferencesStore } from "./gate/config.js";
export type { GatePreferencesStore };
/** Mutable access-key store; the default persists into ~/.pi/pix.json. */
export interface GatePasswordStore {
...
export interface GateDeps {
  ...
  /** Access-key writer used by PUT /v1/gate/password. */
  passwordStore?: GatePasswordStore;
  /** True when PIX_PASSWORD overrides the file (the key cannot be changed here). */
  envPasswordManaged?: () => boolean;
...
export interface SessionHistoryReadClient {
  ...
  /**
   * Selected-branch context read (Protocol v2). `leafId` pins the branch;
   * `before` is an exclusive, stable projected entryId cursor (omitted = newest
   * page); an explicit `limit` is the page size (bounded 1..200) while an
   * OMITTED limit returns the complete selected projected active branch.
   * `deferThinking`/`deferMedia` (direct source-history parity) project
   * deferred placeholders / truthful media-omission summaries so the initial
   * history payload stays small; both default to full inline content.
   */
  context(
    sessionId: string,
    options?: {
   ...
   * (direct source-history parity). Backed by the `sessions.thinking` RPC — a
   * pure persisted-JSONL catalog read that can never activate a Worker.
   * Fail-closed: unknown session/entry/block → not_found-shaped error.
   */
  thinking(sessionId: string, entryId: string, blockIndex: number): Promise<unknown>;
  /**
...
/**
 * Global models catalog (settings-cleanup product decision: agent-dir
 * models.json + global settings only; NOT project-scoped). Composition wires
 * the real adapter catalog; tests inject a fake. Returns `unknown` so this
 * foundation module stays free of runtime-core/adapter imports.
 */
...
/** Global models.json editor; credentials are write-only and never returned. */
export interface CatalogModelsMutationSeam {
  readConfig(): Promise<unknown>;
  writeConfig(input: unknown): Promise<unknown>;
  discoverModels(input: unknown): Promise<unknown>;
}

/** Raw-text global settings.json editor seam (agent-dir scope). */
...
export interface PreferencesDeps {
  /** Injected preferences store (default: ~/.pi/pix.json `preferences`). */
  store?: GatePreferencesStore;
  /** Maximum accepted PUT body size (default 128 KiB). */
  bodyLimitBytes?: number;
}

export interface HostDeps {
  /** Server-side user preferences surface (/v1/preferences). */
  ...
  sessions?: {
    ...
    /**
     * Phase 6A: AllowedRoots used to classify session list/detail
     * `workspaceAccess`. Production MUST pass the same service as
     * `resources.allowedRoots`. Omitted ⇒ history_only (never guessed
     * authorized). Classification never expands or mutates roots.
     */
    roots?: AllowedRootService;
...
/** Live gate recheck for an already-upgraded runtime WS. Never carries cookie/password. */
export type WsGateValidity =
  | { readonly ok: true }
  | { readonly ok: false; readonly error: { readonly code: "unauthorized" | "unavailable" | "forbidden"; readonly message: string; readonly retryable: false } };
...
export interface WsSession {
  ...
  /**
   * Live gate-validity closure from the existing gate owner. Production WS
   * sessions always carry one; missing means new lifecycle features must not
   * be advertised. Direct FakeSession tests inject a safe verifier.
   */
  verifyGate?(): WsGateValidity | Promise<WsGateValidity>;
