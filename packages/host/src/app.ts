...
import { createEnvGateConfigSource, createFileGatePasswordStore, createFileGatePreferencesStore, createNormalizedGateConfigSource } from "./gate/config.js";
...
export function createHostApp(deps: HostDeps = {}): HostApp {
  ...
  const gateDeps: GateDeps = {
    ...
    config: gateSource,
    revocations,
    ...(deps.gate?.passwordStore ? {} : { passwordStore: createFileGatePasswordStore() }),
    ...(deps.gate?.envPasswordManaged ? {} : { envPasswordManaged: () => process.env.PIX_PASSWORD !== undefined }),
  };
  ...
  // 1b. JSON/JS/CSS response compression (scoped: never HTML, never SSE)
  ...
  registerPreferencesRoutes(app, {
    preferencesStore: deps.preferences?.store ?? createFileGatePreferencesStore(),
    ...(deps.preferences?.bodyLimitBytes !== undefined ? { bodyLimitBytes: deps.preferences.bodyLimitBytes } : {}),
    logger,
  });
  registerHealthRoutes(app, deps);
  registerBootstrapRoutes(app, deps, gateDeps);
  if (deps.sessions) {
    const sessionRoots = deps.sessions.roots ?? deps.resources?.allowedRoots;
  ...
  const wsGuardOptions = {
    logger,
    gateConfig: gateDeps.config,
    revocations: gateDeps.revocations!,
    ...(gateDeps.cookieName !== undefined ? { cookieName: gateDeps.cookieName } : {}),
    ...(gateDeps.requireForLan !== undefined ? { requireForLan: gateDeps.requireForLan } : {}),
    ...(gateDeps.now !== undefined ? { now: gateDeps.now } : {}),
