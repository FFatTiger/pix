 ...
 * {@link SessionHistoryReadClient} seam: it wraps exactly the read-only
 * catalog RPCs (`sessions.list` / `sessions.read` / `sessions.context` /
 * `sessions.thinking` / `sessions.tree`) and never the runtime lifecycle (no
 ...
 * authentication failure ⇒ 503 on these routes.
 *
...
export function createSessiondSessionsClient(
  ...
  return {
    ...
    async projects(params) {
      return client.call("projects.list", params);
    },
    ...
    async thinking(sessionId, entryId, blockIndex) {
      // Read-only exact-identity deferred-thinking resolution: the sessiond
      // catalog handler reads persisted JSONL with zero runtime involvement.
      return client.call("sessions.thinking", { sessionId, entryId, blockIndex });
    },
