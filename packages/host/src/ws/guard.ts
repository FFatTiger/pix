...
import type { GateConfigSource, HostLogger, RuntimeWsSeam, SessionRevocationStore, WsSession } from "../types.js";
import { DEFAULT_GATE_COOKIE_NAME } from "../gate/token.js";
import { createLiveGateVerifier } from "../gate/ws-validity.js";
...
export interface WsGuardOptions {
  ...
  logger?: HostLogger;
  /** Same normalized GateConfig source HTTP gate middleware uses. */
  gateConfig?: GateConfigSource;
  /** Same revocation store HTTP gate middleware uses. */
  ...
  return upgradeWebSocket((c: Context<HostEnv>) => {
    ...
    return {
      ...
      onMessage(evt: MessageEvent, ws: import("hono/ws").WSContext<import("ws").WebSocket>) {
        ...
        const session: WsSession = createWsSession(c.req.url, ws, options, c);
        try {
...
function createWsSession(
  ...
  if (options.gateConfig && options.revocations && context) {
    session.verifyGate = createLiveGateVerifier({
      config: options.gateConfig,
      revocations: options.revocations,
      cookieToken: getCookie(context, options.cookieName ?? DEFAULT_GATE_COOKIE_NAME),
      url,
      mode: context.get("hostMode"),
      ...(options.requireForLan === undefined ? {} : { requireForLan: options.requireForLan }),
