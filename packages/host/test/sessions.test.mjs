...
function fakeClient(impl) {
  return {
    ...
    async thinking(id, entryId, blockIndex) {
      return impl.thinking(id, entryId, blockIndex);
    },
    async tree(id) {
      return impl.tree(id);
    },
...
test("GET /v1/projects is an independent numbered resource", async () => {
  let captured;
  const app = appWith(fakeClient({
    async list() { return { sessions: [] }; },
    async projects(params) {
      captured = params;
      return { projects: [{ projectRoot: "/p", representativeCwd: "/p", sessionCount: 7, latestActivity: 10 }], page: params.page, pageSize: params.pageSize, total: 1, totalPages: 1, catalogRevision: 2 };
    },
    async read() { return { ...header() }; },
    async context() { return { sessionId: "s1", entries: [], pageInfo: { hasMore: false } }; },
  }));
  const response = await call(app, "/v1/projects?page=2&pageSize=10");
  assert.equal(response.status, 200);
  assert.deepEqual(captured, { page: 2, pageSize: 10 });
  assert.deepEqual((await response.json()).projects[0], { projectRoot: "/p", representativeCwd: "/p", sessionCount: 7, latestActivity: 10 });
});

test("GET /v1/sessions forwards page/pageSize/cwd/projectRoot params", async () => {
  let captured;
  const app = appWith(
    fakeClient({
      async list(params) {
  ...
  const res = await call(app, "/v1/sessions?page=3&pageSize=5&cwd=/p&projectRoot=/root");
  assert.equal(res.status, 200);
...
test("empty cwd is treated as no filter", async () => {
  ...
  await call(app, "/v1/sessions?cwd=&projectRoot=&page=&pageSize=");
  assert.deepEqual(captured, { page: 1, pageSize: 50 });
});

test("GET /v1/sessions/:id returns header authority without duplicating transcript entries", async () => {
  const app = appWith(
    fakeClient({
      async list() {
        return { sessions: [] };
  ...
  assert.equal("entries" in body.session, false, "history entries belong only to the bounded context endpoint");
});
...
test("GET /v1/sessions/:id/context returns context and forwards leafId", async () => {
  ...
  assert.deepEqual(body.context.settings, {
    model: { provider: "openai", modelId: "gpt-5" },
    thinkingLevel: "high",
  });
});

for (const contextTokens of [263_711, 0, null, undefined]) {
  test(`GET /v1/sessions/:id/context preserves the adapter numerator ${String(contextTokens)}`, async () => {
    const projected = {
      sessionId: "s1",
      leafId: "latest",
      entries: [ENTRY],
      settings: { model: { provider: "p", modelId: "m" }, thinkingLevel: "high" },
    ...
    const app = appWith(fakeClient({
      async list() { return { sessions: [] }; },
      async read() { return header(); },
      async context() { return projected; },
    }));
    const response = await call(app, "/v1/sessions/s1/context?deferThinking=1&deferMedia=1");
    assert.equal(response.status, 200);
    assert.deepEqual((await response.json()).context, projected,
      "HTTP projection must retain token estimate, model and leaf together, including explicit null/zero");
  });
}
...
test("GET /v1/sessions/:id/context forwards canonical source defer flags and rejects other spellings", async () => {
  let captured;
  const app = appWith(
    fakeClient({
      async list() { return { sessions: [] }; },
  ...
  const ok = await call(app, "/v1/sessions/s1/context?deferThinking=1&deferMedia=1");
  assert.equal(ok.status, 200);
  assert.deepEqual(captured, { deferThinking: true, deferMedia: true });
  for (const query of ["deferThinking=true", "deferThinking=0", "deferMedia=yes", "deferMedia="]) {
    const res = await call(app, `/v1/sessions/s1/context?${query}`);
    assert.equal(res.status, 400, query);
    assert.equal((await res.json()).code, "INVALID_QUERY");
  }
});

test("GET deferred thinking resolves exact identity and projects the source response", async () => {
  let captured;
  const app = appWith(
    fakeClient({
      async list() { return { sessions: [] }; },
      async read() { return { ...header() }; },
      async context() { return { sessionId: "s1", entries: [], pageInfo: { hasMore: false } }; },
      async thinking(sessionId, entryId, blockIndex) {
        captured = { sessionId, entryId, blockIndex };
        return { sessionId, entryId, blockIndex, thinking: "persisted reasoning" };
      },
    }),
  );
  const res = await call(app, "/v1/sessions/s1/entries/e9/thinking?blockIndex=2");
  assert.equal(res.status, 200);
  assert.deepEqual(captured, { sessionId: "s1", entryId: "e9", blockIndex: 2 });
  assert.deepEqual(await res.json(), { thinking: "persisted reasoning", entryId: "e9" });
  for (const path of [
    "/v1/sessions/s1/entries/e9/thinking",
    "/v1/sessions/s1/entries/e9/thinking?blockIndex=-1",
    "/v1/sessions/s1/entries/e9/thinking?blockIndex=1.5",
  ]) {
    assert.equal((await call(app, path)).status, 400, path);
  }
});

test("deferred thinking maps identity misses to fixed 404 and unavailable failures to sanitized 503", async () => {
  for (const [error, status, code] of [
    [{ code: "not_found", message: "secret entry e9" }, 404, "SESSION_ENTRY_NOT_FOUND"],
    [{ code: "unavailable", message: "secret endpoint" }, 503, "SESSIONS_UNAVAILABLE"],
  ]) {
    const app = appWith(fakeClient({
      async list() { return { sessions: [] }; },
      async read() { return { ...header() }; },
      async context() { return { sessionId: "s1", entries: [], pageInfo: { hasMore: false } }; },
      async thinking() { throw error; },
    }));
    const res = await call(app, "/v1/sessions/s1/entries/e9/thinking?blockIndex=0");
...
test("Host compresses large JSON responses when gzip is accepted", async () => {
  const large = "x".repeat(8 * 1024);
  const app = appWith(fakeClient({
    async list() { return { sessions: [header({ firstMessage: large })] }; },
    async read() { return { ...header() }; },
    async context() { return { sessionId: "s1", entries: [], pageInfo: { hasMore: false } }; },
  }));
  ...
  const decoded = JSON.parse(gunzipSync(Buffer.from(await res.arrayBuffer())).toString("utf8"));
  assert.equal(decoded.sessions[0].firstMessage, large);
});
...
test("pageSize out of range → 400", async () => {
  const app = appWith(
    fakeClient({
      async list() {
        return { sessions: [] };
  ...
  const tooBig = await call(app, `/v1/sessions?pageSize=${SESSIONS_MAX_PAGE_SIZE + 1}`);
  assert.equal(tooBig.status, 400);
  const zero = await call(app, "/v1/sessions?pageSize=0");
  assert.equal(zero.status, 400);
  const nonInt = await call(app, "/v1/sessions?pageSize=1.5");
  assert.equal(nonInt.status, 400);
});

test("page is one-based and projects pageSize is independently bounded", async () => {
  const app = appWith(
    fakeClient({
      async list() {
        return { sessions: [] };
  ...
  assert.equal((await call(app, "/v1/sessions?page=0")).status, 400);
  assert.equal((await call(app, "/v1/sessions?page=-1")).status, 400);
  assert.equal((await call(app, `/v1/projects?pageSize=${PROJECTS_MAX_PAGE_SIZE + 1}`)).status, 400);
});
...
test("non-canonical integer forms are rejected (1e3 / 0x10 / sign / decimal / whitespace / leading zero)", async () => {
  ...
  for (const value of ["1e3", "0x10", "+5", "-1", "1.5", " 5 ", "5 ", " 5", "007", "01"]) {
    const res = await call(app, `/v1/sessions?page=${encodeURIComponent(value)}`);
    assert.equal(res.status, 400, `expected 400 for page=${value}`);
    assert.equal((await res.json()).code, "INVALID_QUERY");
  }
  // Empty string remains "absent" (documented empty-as-absent behavior).
  const empty = await call(app, "/v1/sessions?page=&pageSize=");
  assert.equal(empty.status, 200);
  assert.deepEqual(captured, { page: 1, pageSize: 50 });
  const ok = await call(app, "/v1/sessions?page=10&pageSize=25");
  assert.equal(ok.status, 200);
});
