...
test("live gate verifier attached to WS session rejects after logout", async () => {
  let captured;
  const runtimeWs = {
    attach(session) {
      captured = session;
  ...
  try {
    const login = await fetch(`http://127.0.0.1:${handle.port}/v1/gate/login`, {
      method: "POST",
    ...
    assert.ok(cookie);
    const ws = await connect(handle.port, { headers: { cookie: cookie.split(";", 1)[0] } });
    ws.send("hello");
    await new Promise((resolve) => setTimeout(resolve, 60));
    assert.equal(typeof captured.verifyGate, "function");
    assert.deepEqual(await captured.verifyGate(), { ok: true });
    const logout = await fetch(`http://127.0.0.1:${handle.port}/v1/gate/logout`, {
      method: "POST",
      headers: { host: "127.0.0.1", cookie: cookie.split(";", 1)[0] },
    });
    assert.equal(logout.status, 200);
    const after = await captured.verifyGate();
