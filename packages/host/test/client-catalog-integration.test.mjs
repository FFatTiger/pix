...
test("Host catalog shapes match Client strict schemas", async () => {
  ...
  const models = await app.request(`http://localhost/v1/models?cwd=${cwd}`, { headers: headers() });
...
test("project catalog endpoints require cwd; providers do not", async () => {
...
test("D3B catalog mutation paths are not mounted", async () => {
  ...
  const ok = await app.request(`http://localhost/v1/models?cwd=${encodeURIComponent(root)}`, { headers: headers() });
